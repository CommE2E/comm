...
const rolePermissionsToBitmaskHex = (
  ...
  for (const permission of Object.keys(threadRolePermissions)) {
    const parsed: ParsedThreadPermissionString =
      parseThreadPermissionString(permission);
    if (permissionsMap.has(parsed.permission)) {
      permissionsMap.get(parsed.permission)?.push(parsed);
    } else {
      permissionsMap.set(parsed.permission, [parsed]);
    }
  }

  let bitmask = BigInt(0);
  Object.keys(minimallyEncodedThreadPermissions).forEach((permission, idx) => {
    let permissionBitmask = BigInt(0);
    for (const parsedPermission of permissionsMap.get(permission) ?? []) {
      permissionBitmask |= rolePermissionToBitmask(parsedPermission);
    }
