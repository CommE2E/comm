...
function createThreadChildrenIndex(threads: $ReadOnlyArray<RawThreadInfo>): {
  +[threadID: string]: $ReadOnlyArray<string>,
} {
  const result: { [threadID: string]: Array<string> } = {};
  for (const thread of threads) {
    ...
    if (!result[parentThreadID]) {
      result[parentThreadID] = ([]: Array<string>);
    }
    result[parentThreadID].push(thread.id);
  }
  return result;
}
...
function updateRolesAndPermissions(
  ...
  permissionsUpdater: ThreadRolePermissionsBlob => ThreadRolePermissionsBlob,
): {
  +operations: $ReadOnlyArray<ThreadStoreOperation>,
} {
  const updatedThreads = { ...threads };

  const threadChildrenIDs = createThreadChildrenIndex(values(threads));
  function updateRoles(threadID: string) {
    ...
    for (const roleID of Object.keys(roles)) {
      ...
      if (threadInfo.thick) {
        ...
        updatedPermissionsBlob = permissionsUpdater(
  ...
  function recursivelyUpdatePermissions(
    threadID: string,
    memberToThreadPermissionsFromParent: ?MemberToThreadPermissions,
  ) {
    const threadInfo = updatedThreads[threadID];
    let memberToThreadPermissionsForChildren: MemberToThreadPermissions;
    ...
    if (threadInfo.thick) {
      ...
      } = updateMembers(threadInfo, memberToThreadPermissionsFromParent);
      updatedThreads[threadID] = {
        ...threadInfo,
        members: updatedMembers,
      };
  ...
  for (const thread of values(threads)) {
    if (thread.thick || !thread.parentThreadID) {
  ...
  rootThreadIDs.forEach(threadID =>
    recursivelyUpdatePermissions(threadID, null),
  );
  rootThreadIDs.forEach(threadID =>
