  ...
  return React.useCallback(
    async (
      ...
      if (mainDatabaseVersion === restoredDatabaseVersion) {
        addLog(
          'User Data Restore',
          `Main and restored database versions are equal: ${mainDatabaseVersion}, ` +
            `skipping schema migrations`,
        );
      } else if (mainDatabaseVersion > restoredDatabaseVersion) {
        addLog(
          'User Data Restore',
          `Main database version (${mainDatabaseVersion}) is higher than restored` +
            `database version (${restoredDatabaseVersion}), migrating schema`,
        );
        await sqliteAPI.migrateBackupSchema();
      } else if (mainDatabaseVersion < restoredDatabaseVersion) {
        addLog(
          'User Data Restore',
          `Main database version (${mainDatabaseVersion}) is lower than restored` +
            `database version (${restoredDatabaseVersion}), aborting`,
        );
        throw new Error('backup_is_newer');
      }

      // 3. Check store versions and migrate if needed
      const [mainStoreVersion, restoredStoreVersion] = await Promise.all([
      ...
      if (!mainStoreVersion || !restoredStoreVersion) {
        addLog(
          ...
          `Error when restoring user data, main store version(${
            ...
            restoredStoreVersion ?? 'undefined'
          }) are undefined`,
        );
        return;
      }
      ...
      if (mainStoreVersion === restoredStoreVersion) {
        ...
        addLog(
          'User Data Restore',
          `Main store version (${mainStoreVersion}) is higher than restored` +
            `store version (${restoredStoreVersion}), migrating data`,
        );
        //TODO: call function from D14780
      } else if (mainStoreVersion < restoredStoreVersion) {
        addLog(
          'User Data Restore',
          `Main store version (${mainStoreVersion}) is lower than restored` +
            `store version (${restoredStoreVersion}), aborting`,
        );
        throw new Error('backup_is_newer');
      }
