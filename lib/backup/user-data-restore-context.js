...
import {
  ...
  generateAuxUserOpsToUpdatePeers,
  ...
  const executeRestoreUserData = React.useCallback(
    async (
      ...
      if (restoreBackupState.status === 'user_data_restore_step_completed') {
        const completedStepIndex = restoreUserDataStepsOrder.indexOf(
        ...
        startStepIndex = restoreUserDataStepsOrder.indexOf(
          restoreBackupState.payload.step,
        );
      } else {
      ...
      if (startStepIndex > restoreUserDataStepsOrder.length - 1) {
        startStepIndex = 0;
      }
      ...
      if (startStepIndex <= 4) {
        const removeLocalMessageInfosPromise =
          sqliteAPI.removeLocalMessageInfos(
            isPrimary,
            databaseIdentifier.RESTORED,
          );
        void dispatchActionPromise(
