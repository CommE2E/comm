...
function isFarcasterUserID(userID: string): boolean {
  return userID.startsWith(farcasterUserIDPrefix);
}

function farcasterPersonalConversationID(
  user1FID: number | string,
  user2FID: number | string,
): string {
  return [user1FID, user2FID].sort().join('-');
}

function getFIDFromUserID(userID: string, auxUserInfos: AuxUserInfos): ?string {
  return auxUserInfos[userID]?.fid ?? extractFIDFromUserID(userID);
}
