...
function robotextForMessageInfo(
  ...
  if (messageInfo.type === messageType.CREATE_THREAD) {
    ...
    return `${creator} created an event scheduled for ${date}: ` +
      `"${messageInfo.text}"`;
  } else if (messageInfo.type === messageType.EDIT_ENTRY) {
    const date = prettyDate(messageInfo.date);
    return `${creator} updated the text of an event scheduled for ` +
      `${date}: "${messageInfo.text}"`;
  } else if (messageInfo.type === messageType.DELETE_ENTRY) {
    const date = prettyDate(messageInfo.date);
    return `${creator} deleted an event scheduled for ${date}: ` +
      `"${messageInfo.text}"`;
  } else if (messageInfo.type === messageType.RESTORE_ENTRY) {
    const date = prettyDate(messageInfo.date);
...
function createMessageInfo(
  ...
  } else if (rawMessageInfo.type === messageType.DELETE_ENTRY) {
    return {
  ...
  } else if (rawMessageInfo.type === messageType.RESTORE_ENTRY) {
    return {
...
function rawMessageInfoFromMessageData(
  ...
  } else if (messageData.type === messageType.DELETE_ENTRY) {
    return {
      type: messageType.DELETE_ENTRY,
      id,
      threadID: messageData.threadID,
  ...
  } else if (messageData.type === messageType.RESTORE_ENTRY) {
    return {
      type: messageType.RESTORE_ENTRY,
      id,
      threadID: messageData.threadID,
...
function messageTypeGeneratesNotifs(type: MessageType) {
  return type !== messageType.JOIN_THREAD &&
    type !== messageType.LEAVE_THREAD &&
    type !== messageType.ADD_MEMBERS &&
    type !== messageType.REMOVE_MEMBERS;
}
