...
import { useENSAvatar } from '../hooks/names-cache.js';
import type {
  ClientAvatar,
  ClientEmojiAvatar,
...
function useDisplayUserForThread(
  ...
  return React.useMemo(() => {
    ...
    const fid = extractFIDFromUserID(displayUserIDForThread);
    if (fid) {
      return {
        farcasterID: fid,
      };
    ...
    const userBase = userInfos[displayUserIDForThread];
    const farcasterID = auxUserInfos[displayUserIDForThread]?.fid;

    return {
      ...userBase,
      farcasterID,
    };
  }, [displayUserIDForThread, userInfos, auxUserInfos]);
}
