...
import { protocolNames } from '../../../types/protocol-names.js';
...
import type {
  ...
  DeleteMessageUtils,
  ProtocolPinMessageInput,
  PinMessageUtils,
  ProtocolFetchPinnedMessagesInput,
  FetchPinnedMessagesUtils,
} from '../thread-spec.js';
...
const farcasterThreadProtocol: ThreadProtocol<MemberInfoSansPermissions> = {
  ...
  changeThreadSettings: async (
    ...
    if (changes.avatar && changes.avatar.type === 'non_keyserver_image') {
      promises.push(
        utils.updateFarcasterThreadAvatar({
          conversationID,
          action: { type: 'set', imageURL: changes.avatar.blobURI },
        }),
      );
    } else if (changes.avatar && changes.avatar.type === 'remove') {
      promises.push(
        utils.updateFarcasterThreadAvatar({
          conversationID,
          action: { type: 'remove' },
  ...
  deleteMessage: async (
    ...
    const { farcasterDeleteMessage, dispatch } = utils;
    ...
    dispatch({
      type: processFarcasterOpsActionType,
      payload: { rawMessageInfos: [deleteMessage], updateInfos: [] },
    });
  },
  ...
  fetchPinnedMessages: async (
    input: ProtocolFetchPinnedMessagesInput,
    utils: FetchPinnedMessagesUtils,
  ) => {
    const { threadInfo } = input;
    const { fetchMessage } = utils;

    const pinnedMessageIDs = threadInfo.pinnedMessageIDs ?? [];

    const pinnedMessagePromises = pinnedMessageIDs.map(messageID =>
      fetchMessage(messageID),
    );

    const results = await Promise.all(pinnedMessagePromises);
    const pinnedMessages = results.filter(Boolean);
