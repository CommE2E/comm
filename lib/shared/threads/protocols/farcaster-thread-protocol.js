...
const farcasterThreadProtocol: ThreadProtocol<MemberInfoSansPermissions> = {
  sendTextMessage: async (
    ...
    //TODO: add check if this was first ever message to avoid fetching
    // conversation on each message
    if (threadInfo.type === farcasterThreadTypes.FARCASTER_PERSONAL) {
  ...
  sendMultimediaMessage: async (
    ...
    let request;
    if (threadInfo.type === farcasterThreadTypes.FARCASTER_PERSONAL) {
      //TODO: re-use code from sendTextMessage
    } else {
      const conversationID = conversationIDFromFarcasterThreadID(threadInfo.id);
      request = {
        groupId: conversationID,
  ...
  createRealThreadFromPendingThread: async (
    ...
    if (threadTypeIsPersonal(threadInfo.type)) {
      ...
      const conversationID = [viewerFid, otherUserFid].sort().join('-');

      // We construct thread here to make it work with ours design,
      // when sending message thread is going to be fetched,
      // so Farcaster remains as source of truth
      dispatch({
          ...
          updateInfos: [
            {
              ...
              threadInfo: createFarcasterRawThreadInfoPersonal({
                ...threadInfo,
                id: farcasterThreadIDFromConversationID(conversationID),
              }),
              rawMessageInfos: [],
