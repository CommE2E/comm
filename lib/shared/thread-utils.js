...
import type { ProtocolName, ThreadProtocol } from './threads/thread-spec.js';
import {
  threadSpecs,
...
function useExistingThreadInfoFinder(
  ...
  const selector =
    createPendingToRealizedThreadIDsSelectorMemoized(selectedProtocol);
  const pendingToRealizedThreadIDs = useSelector(state =>
    selector(state.threadStore.threadInfos),
  );
  return React.useCallback(
    (params: ExistingThreadInfoFinderParams): ?ThreadInfo => {
      ...
      console.log('useExistingThreadInfoFinder', baseThreadInfo.id);
      ...
      if (searching) {
        const protocol = selectedProtocol
          ? (getProtocolByName(selectedProtocol) ?? dmThreadProtocol)
          : dmThreadProtocol;

        console.log(
          'useExistingThreadInfoFinder protocolName',
      ...
      console.log(
        'useExistingThreadInfoFinder pendingThreadID',
        pendingThreadID,
        selectedProtocol,
      ...
      if (params.allUsersSupportFarcasterThreads && supportsFarcasterDCs) {
