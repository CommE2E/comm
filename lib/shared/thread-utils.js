...
function canCreateSidebar(
  threadInfo: ThreadInfo,
  messageCreatorRelationship: ?UserRelationshipStatus,
) {
...
function useSidebarExistsOrCanBeCreated(
  ...
  const messageCreatorUserInfo = useSelector(
    (state) => state.userStore.userInfos[messageItem.messageInfo.creator.id],
  );
  const canCreateSidebarFromMessage = canCreateSidebar(
    threadInfo,
