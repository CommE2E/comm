...
function getChatMentionCandidates(threadInfos: {
  ...
  communityThreadIDsByThread: { +[id: string]: string },
} {
  const result = {};
  const visitedGenesisThreads = new Set();
  const communityThreadIDsByThread = {};
    ...
    if (threadInfos[currentThreadCommunity].type === threadTypes.GENESIS) {
      ...
      if (
        ...
        for (const threadInfo of threadTraversePath) {
          ...
          communityThreadIDsByThread[threadInfo.id] =
            lastThreadInTraversePath.id;
        }
      } else {
        if (!communityThreadIDsByThread[lastThreadInTraversePathParentID]) {
          communityThreadIDsByThread[lastThreadInTraversePathParentID] =
            lastThreadInTraversePathParentID;
        }
        const lastThreadInTraversePathParentCommunityThreadID =
          communityThreadIDsByThread[lastThreadInTraversePathParentID];
        if (!result[lastThreadInTraversePathParentCommunityThreadID]) {
  ...
  return { chatMentionCandidatesObj: result, communityThreadIDsByThread };
}
...
function useChatMentionCandidatesObjAndResolvedThreadInfos(): {
  ...
  const { chatMentionCandidatesObj, communityThreadIDsByThread } =
    React.useMemo(
...
function useThreadChatMentionCandidates(
  ...
  return React.useMemo(() => {
    let communityID,
      result = {};
    if (communityThreadIDsByThread[threadInfo.id]) {
      communityID = communityThreadIDsByThread[threadInfo.id];
    } else {
...
function useChatMentionSearchIndex(): {
  ...
  const { chatMentionCandidatesObj, resolvedThreadInfos } =
    useChatMentionCandidatesObjAndResolvedThreadInfos();
  return React.useMemo(() => {
    const result = {};
    const visitedThreads = new Set();
    for (const threadID in chatMentionCandidatesObj) {
      if (visitedThreads.has(threadID) || !resolvedThreadInfos[threadID]) {
        continue;
      }
      ...
      for (const key in chatMentionCandidatesObj[threadID]) {
        result[key] = searchIndex;
        searchIndex.addEntry(
          chatMentionCandidatesObj[threadID][key].id,
          chatMentionCandidatesObj[threadID][key].uiName,
        );
        visitedThreads.add(chatMentionCandidatesObj[threadID][key].id);
      }
    }
    return result;
  }, [chatMentionCandidatesObj, resolvedThreadInfos]);
}
