...
function getCurrentUser(
  ...
  if (
    !threadFrozenDueToBlock(threadInfo, parentThreadInfo, viewerID, userInfos)
  ) {
...
function threadIsWithBlockedUserOnly(
  threadInfo: RawThreadInfo | ThreadInfo,
  parentThreadInfo: ?RawThreadInfo | ?ThreadInfo,
  viewerID: ?string,
  userInfos: UserInfos,
  checkOnlyViewerBlock?: boolean,
): boolean {
  // We are skipping non-personal threads and
  // sidebars which parent threads are non-personal.
  if (
    ((threadInfo.type === threadTypes.SIDEBAR &&
      parentThreadInfo &&
      parentThreadInfo.type !== threadTypes.PERSONAL) ||
...
function useExistingThreadInfoFinder(
  ...
  const parentThreadInfo: ?ThreadInfo = useSelector(state =>
    baseThreadInfo ? threadInfoSelector(state)[baseThreadInfo.id] : null,
  );
