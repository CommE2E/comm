...
function getChatMentionCandidates(threadInfos: {
  ...
  for (const currentThreadID in threadInfos) {
    ...
    if (!currentThreadCommunity) {
      result[currentThreadID] = [currentThreadInfo];
      continue;
    }
    ...
    if (threadInfos[currentThreadCommunity].type === threadTypes.GENESIS) {
      ...
      const { parentThreadID: lastThreadParentThreadID } =
        threadTraversePath[threadTraversePath.length - 1];
      invariant(
        lastThreadParentThreadID,
        'parentThreadID should be defined in the last thread in threadTraversePath',
      );
      if (threadInfos[lastThreadParentThreadID].type === threadTypes.GENESIS) {
        for (const { id } of threadTraversePath) {
          result[id] = threadTraversePath;
        }
      } else {
        for (const threadInfo of threadTraversePath) {
          result[lastThreadParentThreadID].push(threadInfo);
          result[threadInfo.id] = result[lastThreadParentThreadID];
        }
      }
      continue;
    }
    result[currentThreadCommunity].push(currentThreadInfo);
    result[currentThreadID] = result[currentThreadCommunity];
  }
  return result;
}

function useChatMentionCandidatesSearchIndex(currentThreadID: string): {
  +[id: string]: SearchIndex,
} {
  const threadInfos = useSelector(threadInfoSelector);
  ...
  return React.useMemo(() => {
    ...
    const chatMentionCandidates = getChatMentionCandidates(resolvedThreadInfos);
    for (const threadID in chatMentionCandidates) {
      if (visitedThreads.has(threadID)) {
        continue;
      }
      const searchIndex = new SearchIndex();
      for (const threadInfo of chatMentionCandidates[threadID]) {
        result[threadInfo.id] = searchIndex;
        searchIndex.addEntry(threadInfo.id, threadInfo.uiName);
        visitedThreads.add(threadInfo.id);
      }
    }
    return result;
  }, [resolvedThreadInfos])[currentThreadID];
}
...
export {
  ...
  useChatMentionCandidatesSearchIndex,
};
