...
function createPendingPersonalThread(
  ...
  const threadType = flag ? threadTypes.PERSONAL : threadTypes.GENESIS_PERSONAL;
...
function useUserProfileThreadInfo(userInfo: ?UserInfo): ?UserProfileThreadInfo {
  ...
  React.useEffect(() => {
    void (async () => {
      ...
      const [res] = await usersSupportsThickThreads([userInfo.id]);
      setSupportThickThreads(!!res);
    })();
