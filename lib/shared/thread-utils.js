...
function threadInChatList(
  ...
  const isFarcaster = threadInfo?.type
    ? threadSpecs[threadInfo.type].protocol().protocolName === 'Farcaster DC'
    : false;
  return (
    (viewerIsMember(threadInfo) || isFarcaster) &&
...
function getPendingThreadID(
  ...
  if (sourceMessageID) {
    const config = threadSpecs[threadType].protocol().sidebarConfig;
    invariant(config, `Thread type ${threadType} doesn't support sidebars`);
    const prefix = config.pendingSidebarURLPrefix;
...
export type CreatePendingThreadArgs = {
  +viewerID: string,
  +threadType: ThreadType,
  +members: $ReadOnlyArray<BasicUserInfo>,
...
function createPendingThread(args: CreatePendingThreadArgs): ThreadInfo {
  const { viewerID, threadType, members, sourceMessageID } = args;
  const now = Date.now();
  ...
  for (const member of members) {
    ...
    userInfos[id] = { id, username, avatar };
  }
...
function createPendingPersonalOrPrivateThread(
  ...
  const members: Array<BasicUserInfo> = [loggedInUserInfo];
  let threadType;
...
type RawThreadInfoOptions = {
  ...
  +messageDeletionUnsupported?: boolean,
};
...
function rawThreadInfoFromServerThreadInfo(
  ...
  const filterDeleteMessagePermissions = options?.messageDeletionUnsupported;
  ...
  const filterThreadPermissions = (
    ...
    return _omitBy(
      (v, k) =>
        ...
        (filterDeleteMessagePermissions &&
          (k.endsWith(threadPermissions.DELETE_OWN_MESSAGES) ||
            k.endsWith(threadPermissions.DELETE_ALL_MESSAGES))),
    )(innerThreadPermissions);
  };
  ...
  invariant(
    !minimallyEncodedRawThreadInfoWithMemberPermissions.thick &&
      !minimallyEncodedRawThreadInfoWithMemberPermissions.farcaster,
...
function threadLabel(threadType: ThreadType): string {
  return threadSpecs[threadType].threadLabel;
}
...
type ExistingThreadInfoFinderParams =
  ...
  | {
      +searching: true,
      +userInfoInputArray: $ReadOnlyArray<BasicUserInfo>,
      +matchProtocol: ?ProtocolName,
      +pendingThreadProtocol: ProtocolName,
    };
...
function useExistingThreadInfoFinder(
  ...
  return React.useCallback(
    (params: ExistingThreadInfoFinderParams): ?ThreadInfo => {
      ...
      let pendingThreadID;
      if (params.searching) {
        const { userInfoInputArray, matchProtocol, pendingThreadProtocol } =
          params;
        const protocol = getProtocolByName(pendingThreadProtocol);
        invariant(protocol, `unsupported protocol ${pendingThreadProtocol}`);
        ...
        pendingThreadID = getPendingThreadID(
          ...
          sourceMessageID,
          threadSpecs[baseThreadInfo.type].protocol().protocolName,
        );
      }
      ...
      const { userInfoInputArray, pendingThreadProtocol } = params;
      const protocol = getProtocolByName(pendingThreadProtocol);
      invariant(protocol, `unsupported protocol ${pendingThreadProtocol}`);
...
function patchThreadInfoToIncludeMentionedMembersOfParent(
  ...
  const members: BasicUserInfo[] = threadInfo.members
    .map(({ id, username }) =>
      username ? ({ id, username }: BasicUserInfo) : null,
    )
    .filter(Boolean);
  ...
  const config = threadSpecs[parentThreadInfo.type].protocol().sidebarConfig;
  invariant(config, `Thread type ${parentThreadInfo.type} can't have sidebars`);
  const threadType = config.sidebarThreadType;
...
function useUserProfileThreadInfo(
