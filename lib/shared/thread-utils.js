...
function getPendingThreadOtherUsers(threadInfo: ThreadInfo | RawThreadInfo) {
  ...
  invariant(
    otherUserIDs || threadInfo.type === threadTypes.SIDEBAR,
    'Pending threads should contain other members id in its id',
  );
...
function useRealThreadCreator(
  ...
  const serverThreadID = React.useRef<?string>(null);
  ...
  return React.useCallback(async () => {
    ...
    const isThreadCreated = threadID && !threadIsPending(threadID);
    if (isThreadCreated || !threadInfo) {
      return threadID;
    }
    if (!serverThreadID.current) {
      serverThreadID.current = await createRealThreadFromPendingThread(
        threadInfo,
...
function useThreadCandidates(threadInfos: { [id: string]: ThreadInfo }) {
...
function useSidebarCandidate(sourceMessageID: ?string) {
...
function useLatestThreadInfo(
  originalThreadInfo: ThreadInfo,
  searching: ?boolean,
  userInfoInputArray: $ReadOnlyArray<AccountUserInfo>,
  sourceMessageID: ?string,
) {
  const threadInfos = useSelector(threadInfoSelector);
  const threadCandidates = useThreadCandidates(threadInfos);
  ...
  return React.useMemo((): ?ThreadInfo => {
