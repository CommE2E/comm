...
const subscriptionExists = createSelector(
  (state: BaseAppState<BaseNavInfo>) => state.calendarInfos,
  (calendarInfos: {[id: string]: CalendarInfo}) =>
    _.some(calendarInfos, 'subscribed'),
);

const currentNavID = createSelector(
  (state: BaseAppState<BaseNavInfo>) => state.navInfo,
  (state: BaseAppState<BaseNavInfo>) => state.calendarInfos,
  subscriptionExists,
  (
    navInfo: BaseNavInfo,
...
function mergeNewEntriesIntoStore<U: BaseNavInfo, T: BaseAppState<U>>(
  updateStore: UpdateStore<U, T>,
...
async function fetchEntriesAndUpdateStore<U: BaseNavInfo, T: BaseAppState<U>>(
  year: number,
  month: number,
  navID: string,
  updateStore: UpdateStore<U, T>,
