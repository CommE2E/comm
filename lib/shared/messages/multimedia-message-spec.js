...
import { messageTypes, assertMessageType } from '../../types/message-types';
import type {
  ImagesMessageData,
...
import { translateClientDBMediaInfoToImage } from '../../utils/message-ops-utils';
import {
...
> = Object.freeze({
  ...
  rawMessageInfoFromClientDB(
    ...
    invariant(
      assertMessageType(parseInt(clientDBMessageInfo.type)) ===
        messageTypes.IMAGES,
      'message must be of type IMAGES',
    );
    invariant(
    ...
    const translatedMedia = clientDBMessageInfo.media_infos.map(
    ...
    if (clientDBMessageInfo.local_id) {
      rawImagesMessageInfo.localID = clientDBMessageInfo.local_id;
    }
    if (clientDBMessageInfo.id !== clientDBMessageInfo.local_id) {
      rawImagesMessageInfo.id = clientDBMessageInfo.id;
    }
    return rawImagesMessageInfo;
  },
  ...
  shimUnsupportedMessageInfo(
    ...
    if (rawMessageInfo.type === messageTypes.IMAGES) {
      ...
      if (hasMinCodeVersion(platformDetails, 148)) {
        return shimmedRawMessageInfo;
      }
