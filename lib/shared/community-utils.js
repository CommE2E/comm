...
import { useAddKeyserver, useIsKeyserverURLValid } from './keyserver-utils.js';
...
function useJoinCommunity(params: UseJoinCommunityParams): () => Promise<void> {
  ...
  React.useEffect(() => {
    void (async () => {
      ...
      if (!isValid || !keyserverURL || !keyserverID) {
        setLinkStatus?.('invalid');
        ongoingJoinData.reject();
        setOngoingJoinData(null);
        return;
      }
      await addKeyserver(keyserverID, defaultKeyserverInfo(keyserverURL));
    })();
  }, [
    addKeyserver,
    dispatch,
