...
function useMemberMapForUserMentions(
  ...
  const resolvedMembers = useENSNames(members, useENSNamesOptions);

  for (let i = 0; i < members.length; i++) {
    const member = members[i];
    const resolvedMember = resolvedMembers[i];

    const { username: rawUsername, role } = member;
    const { username: resolvedUsername } = resolvedMember;

    if (role && rawUsername) {
      membersMap.set(rawUsername.toLowerCase(), member.id);
    }

    if (resolvedUsername && resolvedUsername !== rawUsername) {
      membersMap.set(resolvedUsername.toLowerCase(), member.id);
    }
  }
