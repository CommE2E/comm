...
function unshimFunc(
  ...
  unshimTypes: $ReadOnlySet<MessageType>,
): RawMessageInfo {
  ...
  // Unshimming the plain messages should always consist of converting it,
  // so we can't use the existing mechanism where we just return the message
  if (messageInfo.unsupportedMessageInfo.type === messageTypes.PLAIN) {
    return messageInfo;
  }
...
const localUnshimTypes: $ReadOnlySet<MessageType> = new Set([
  messageTypes.IMAGES,
...
async function unshimDMOperations<N: BaseNavInfo, T: BaseAppState<N>>(
  ...
  try {
    ...
    const dmOperationStoreOperations = shimmedOperations.map(op => ({
      type: 'replace_dm_operation',
      payload: {
        id: op.id,
        // Updating the operation type to be 'unshimmed' to distinguish it when
        // reading database content from operations that are still unsupported.
    ...
    return {
      ...
      ops: { dmOperationStoreOperations },
    };
  } catch (e) {
    console.log(e);
