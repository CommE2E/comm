...
const createSidebarSpec: DMOperationSpec<DMCreateSidebarOperation> =
  Object.freeze({
    processDMOperation: async (
      ...
      const sourceMessageInfo = createMessageInfo(
        sourceMessage,
        viewerID,
        utilities.userInfos,
        utilities.threadInfos,
      );
      const parentThreadInfo = utilities.threadInfos[parentThreadID];
      let messageTitle = null;
      if (sourceMessageInfo && parentThreadInfo) {
        invariant(
          sourceMessageInfo.type !== messageTypes.SIDEBAR_SOURCE,
          "A sidebar can't be created from another sidebar's source",
        );
        messageTitle = await createSidebarTitle(
          sourceMessageInfo,
          parentThreadInfo,
          utilities.markdownRules,
          utilities.getENSNames,
          utilities.getFCNames,
        );
      }
      const threadName = messageTitle
        ? createSidebarThreadName(messageTitle)
        : null;
      ...
      const notificationsCreationData = {
        ...
        thickRawThreadInfos: {
          ...
          [parentThreadID]: utilities.rawThreadInfos[parentThreadID],
        },
      };
    ...
    canBeProcessed: async (
      ...
      if (utilities.rawThreadInfos[dmOperation.threadID]) {
        console.log(
