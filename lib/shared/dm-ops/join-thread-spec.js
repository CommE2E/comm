...
const joinThreadSpec: DMOperationSpec<DMJoinThreadOperation> = Object.freeze({
  processDMOperation: async (
    ...
    const currentThreadInfo = rawThreadInfos[existingThreadDetails.threadID];
    ...
    if (userIsMember(currentThreadInfo, joinerID)) {
      ...
      const parentThreadInfo = parentThreadID
        ? utilities.rawThreadInfos[parentThreadID]
        : null;
  ...
  canBeProcessed: async (
    ...
    const { viewerID, rawThreadInfos } = utilities;
    if (
      rawThreadInfos[dmOperation.existingThreadDetails.threadID] ||
