...
import { getCreateThickRawThreadInfoInputFromThreadInfo } from '../threads/protocols/dm-thread-protocol.js';
...
function useCreateMessagesToPeersFromDMOp(): (
  ...
  return React.useCallback(
    async (
      ...
      const { viewerID, rawThreadInfos } = utilities;
      if (!viewerID) {
        return [];
      }
      ...
      if (recipients.type === 'self_devices') {
        ...
        const members = rawThreadInfos[recipients.threadID]?.members ?? [];
      ...
      return generateMessagesToPeers(
        plaintext,
        targetPeers,
        dmOpSpecs[operation.type].supportsAutoRetry,
      );
    },
...
function getThreadUpdatesForNewMessages(
  ...
  for (const threadID in messagesByThreadID) {
    ...
    invariant(threadInfo.timestamps, 'Thread should have timestamps');
...
function useSendDMOperationUtils(): $ReadOnly<{
  ...
  const ensCache = React.useContext(ENSCacheContext);
  const getENSNames = ensCache?.getENSNames;
  ...
  const markdownRules = React.useMemo(
    () => getConfig().getDefaultTextMessageRules({}).simpleMarkdownRules,
    [],
  );
...
function useGenerateP2PMessageToOwnDevices(): (
  plaintext: string,
) => Promise<$ReadOnlyArray<OutboundP2PMessage>> {
  const allPeerUserIDAndDeviceIDs = useSelector(getAllPeerUserIDAndDeviceIDs);
  ...
  return React.useCallback(
    async (plaintext: string) => {
      const { deviceID, userID } = await getAuthMetadata();

      const targetPeers = allPeerUserIDAndDeviceIDs
        .filter(peer => peer.userID === userID)
        .filter(peer => peer.deviceID !== deviceID);

      return generateMessagesToPeers(plaintext, targetPeers, true);
    },
    [allPeerUserIDAndDeviceIDs, getAuthMetadata],
  );
}
...
export {
  ...
  useGenerateP2PMessageToOwnDevices,
};
