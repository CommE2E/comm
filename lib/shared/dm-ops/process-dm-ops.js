  ...
  return React.useCallback(
    async (dmOperationSpecification: DMOperationSpecification) => {
      ...
      if (
        ...
        !dmOperationSpecification.supportsAutoRetry &&
        dmOperationSpecification.op.type === dmOperationTypes.SEND_TEXT_MESSAGE
      ) {
...
function useProcessAndSendDMOperation(): (
  ...
  return React.useCallback(
    async (dmOperationSpecification: OutboundDMOperationSpecification) => {
      const { promise } = getDMOpsSendingPromise();
      ...
      if (
        ...
        const sendingPromise: Promise<SendDMOpsPayload> = (async () => {
