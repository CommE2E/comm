  ...
  Object.freeze({
    processDMOperation: async (
      ...
      const { time, messageID, addedUserIDs, existingThreadDetails, editorID } =
        dmOperation;
      const { rawThreadInfos } = utilities;

      let rawMessageInfos: Array<RawMessageInfo> = [];
      let messageDataWithMessageInfos: ?{
        +messageData: AddMembersMessageData,
        +rawMessageInfo: RawMessageInfo,
      } = null;
      if (messageID) {
        messageDataWithMessageInfos =
      ...
      const currentThreadInfo = rawThreadInfos[threadID];
