...
function fullNotifTextForMessageInfo(
  ...
  if (mostRecentType === messageType.TEXT) {
    ...
    return `${stringForUser(messageInfo.creator)} created an event in ` +
      ...
      `${prettyDate(messageInfo.date)}: "${messageInfo.text}"`;
  } else if (mostRecentType === messageType.DELETE_ENTRY) {
    const messageInfo = assertSingleMessageInfo(messageInfos);
    invariant(
      messageInfo.type === messageType.DELETE_ENTRY,
      "messageInfo should be messageType.DELETE_ENTRY!",
    );
    return `${stringForUser(messageInfo.creator)} deleted an event in ` +
      `${notifThreadName(threadInfo)} scheduled for ` +
      `${prettyDate(messageInfo.date)}: "${messageInfo.text}"`;
  } else if (mostRecentType === messageType.RESTORE_ENTRY) {
    const messageInfo = assertSingleMessageInfo(messageInfos);
    invariant(
      messageInfo.type === messageType.RESTORE_ENTRY,
      "messageInfo should be messageType.RESTORE_ENTRY!",
    );
