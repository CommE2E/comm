...
import { threadSpecs, threadTypeIsPersonal } from './threads/thread-specs.js';
...
function baseCreatePendingSidebar(
  ...
  const initialMembers = new Map<string, BasicUserInfo>();
  ...
  return createPendingThread({
    viewerID,
    threadType: config.sidebarThreadType,
...
async function createSidebarTitle(
  sourceMessageInfo:
    | ComposableMessageInfo
    | RobotextMessageInfo
    | ReactionMessageInfo
  ...
  return await getEntityTextAsString(
...
async function createPendingSidebar(
  ...
  const messageTitle = await createSidebarTitle(
    sourceMessageInfo,
    parentThreadInfo,
    markdownRules,
    getENSNames,
...
function useCanCreateSidebarFromMessage(
  ...
  if (
    ...
    !threadSpecs[threadInfo.type].protocol().sidebarConfig
  ) {
    return false;
  }
