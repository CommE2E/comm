...
function getPotentialMemberItems(
  ...
  const appendUserInfo = (userInfo: AccountUserInfo) => {
    ...
    if (
      communityThreadInfo &&
      communityThreadInfo.id !== genesis.id &&
  ...
  // This hack is expected to be removed with ENG-55
  if (parentThreadInfo && parentThreadInfo.id === genesis.id) {
    results = results.filter(
      userInfo => userInfo.relationshipStatus === userRelationshipStatus.FRIEND,
    );
  }
