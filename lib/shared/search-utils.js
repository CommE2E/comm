...
import { getSupportedProtocolsForUser } from './protocol-selection-utils.js';
...
import { getProtocolByName } from './threads/protocols/thread-protocols.js';
...
function usePotentialMemberItems({
  ...
  const shouldExcludeUserFromResult = React.useCallback(
    (userID: string) => {
      ...
      const protocolPresentationDetails = protocol
        ? getProtocolByName(protocol)?.presentationDetails
        : null;
      return !!(
        protocolPresentationDetails?.usersWithoutDeviceListExcludedFromSearchResult &&
        !userHasDeviceList(userID, auxUserInfos)
      );
    },
    [auxUserInfos, excludeUserIDs, protocol],
  );
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      const usersWithProtocol = sortedMembers
        .map(member => {
          const supportedProtocols = getSupportedProtocolsForUser({
            supportsThickThreads: thickThreadUsers.get(member.id),
            supportsFarcasterDCs:
              viewerID !== member.id && farcasterUsers.get(member.id),
          });
