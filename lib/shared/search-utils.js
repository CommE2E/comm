...
import { useProtocolSelection } from '../components/protocol-selection-provider.react.js';
...
function usePotentialMemberItems({
  ...
  React.useEffect(() => {
    ...
    const result = sortedResults.map(
      ({
        ...
        const targetFid =
          auxUserInfos[result.id]?.fid ?? extractFIDFromUserID(result.id);
        const isFarcasterUser = supportsFarcasterDCs && !!targetFid;
        if (
          relationshipStatus &&
          relationshipBlockedInEitherDirection(relationshipStatus)
        ) {
        ...
        // const supportedProtocols: Array<number> = [];
        // protocols().forEach((protocol, index) => {
        //   if (
        //     protocol.doesUserSupportThreadProtocol(result.id, { auxUserInfos })
        //   ) {
        //     supportedProtocols.push(index);
        //   }
        // });
        // result = {
        //   ...result,
  ...
  const [potentialMembers, setPotentialMembers] = useState<UserListItem[]>([]);
  React.useEffect(() => {
    void (async () => {
      ...
      const usersWithProtocol = sortedMembers
        .map(member => {
          ...
          if (thickThreadUsers.get(member.id)) {
            supportedProtocols.push('Comm DM');
          }
          if (farcasterUsers.get(member.id)) {
            supportedProtocols.push('Farcaster DC');
          }
          if (
            selectedProtocol &&
