...
function appendUserInfo({
  ...
  // We want to determine if user has `KNOW_OF` permission in community thread.
  // 1. Find the `MemberInfo` corresponding to `id` in `communityThreadInfo`.
  const memberInfo: ?RelativeMemberInfo = communityThreadInfo?.members.find(
    m => m.id === id,
  );

  // 2. Find the role of the `userInfo.id` in the `communityThreadInfo`.
  const role: ?RoleInfo = memberInfo?.role
    ? communityThreadInfo?.roles[memberInfo.role]
    : null;

  // 3. Decode the bitmask array of `role.permissions`.
  ...
  // 4. Check whether user has the `KNOW_OF` role permission.
