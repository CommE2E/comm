...
function usePotentialMemberItems({
  ...
  +isFarcasterDCsIntegrationEnabled?: boolean,
}): UserListItem[] {
    ...
    const mappedResult = sortedResults.map(
      ({
        ...
        const isFarcasterUser =
          isFarcasterDCsIntegrationEnabled && !!extractFIDFromUserID(result.id);
        if (
          relationshipStatus &&
          relationshipBlockedInEitherDirection(relationshipStatus)
        ) {
  ...
  const getFarcasterUsers = useGetCommFCUsersForFIDs();
  ...
  React.useEffect(() => {
    void (async () => {
      ...
      const fids = usersIDs.map(extractFIDFromUserID).filter(Boolean);
      const fcUserInfos = await getFarcasterUsers(fids);
      const fcCommUserIDs = Array.from(fcUserInfos.keys());

      const userIDsSet = new Set([...usersIDs, ...fcCommUserIDs]);
      const allUserIDs = [...userIDsSet];

      const [thickThreadUsers, farcasterUsers] = await Promise.all([
        usersSupportThickThreads(allUserIDs),
        usersSupportFarcasterDCs(allUserIDs),
      ]);
      const usersWithProtocol = sortedMembers
        .map(member => {
          let memberID = member.id;
          const fid = extractFIDFromUserID(memberID);
          if (fid) {
            const fcCommUser = fcUserInfos.get(fid);
            if (fcCommUser) {
              // replace Farcaster-only userID with actual Comm
              // userID when possible
              memberID = fcCommUser.userID;
            }
          }

          const supportedProtocols: Array<ProtocolName> = [];
          if (thickThreadUsers.get(memberID)) {
            supportedProtocols.push(protocolNames.COMM_DM);
          }
          if (viewerID !== memberID && farcasterUsers.get(memberID)) {
...
type UseSearchUsersOptions = {
  ...
  +searchFarcaster?: ?boolean,
  +skipCommUsers?: ?boolean,
};
function useSearchUsers(
  ...
  const searchFarcaster = !!options?.searchFarcaster;
  const skipCommUsers = !!options?.skipCommUsers;
  ...
  const findFarcasterDCUsers = useGetFarcasterDirectCastUsers();
  ...
  React.useEffect(() => {
    ...
    const searchUsersPromise = (async () => {
      const foundInfos: Array<GlobalAccountUserInfo> = [];

      const identityPromise = (async () => {
        if (identitySearchSocketConnected && !skipCommUsers) {
          try {
      ...
      const farcasterPromise = (async () => {
        if (searchFarcaster) {
          try {
            const fcSearchResult = await findFarcasterDCUsers({
              q: forwardLookupSearchText,
            });
            const userInfos = fcSearchResult.result.users.map(fcUser => ({
              id: userIDFromFID(fcUser.fid.toString()),
              username: fcUser.username
                ? ensNameForFarcasterUsername(fcUser.username)
                : 'anonymous',
              avatar: fcUser.pfp
                ? {
                    type: 'image',
            ...
            console.error('farcaster user search error', err);
          }
        }
      })();

      await Promise.all([identityPromise, farcasterPromise]);

      if (foundInfos.length > 0 || skipCommUsers) {
        setSearchResultsFromServer(foundInfos);
        return;
      }
