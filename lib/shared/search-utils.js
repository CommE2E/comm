...
function useSearchUsers(
  ...
  const [searchResults, setSearchResults] = React.useState<
    $ReadOnlyArray<GlobalAccountUserInfo>,
  >([]);
  const callServerSearchUsers = useLegacyAshoatKeyserverCall(searchUsers);
  const { connected, sendPrefixQuery: callIdentitySearchUsers } =
    useIdentitySearch();
  ...
  React.useEffect(() => {
    ...
    const searchServer = async () => {
      try {
        const { userInfos: serverSearchResult } = await callServerSearchUsers(
    ...
    const searchUsersPromise = (async () => {
      let userInfos;
      if (usingCommServicesAccessToken && connected) {
        try {
          const identitySearchResult = await callIdentitySearchUsers(
            forwardLookupSearchText,
          );
          userInfos = identitySearchResult.map(user => ({
            id: user.userID,
