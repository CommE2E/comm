...
import { urlPrefixSelector } from '../selectors/keyserver-selectors.js';
...
function useIsKeyserverURLValid(keyserverURL?: string): () => Promise<boolean> {
  const ashoatKeyserverUrl = useSelector(urlPrefixSelector(ashoatKeyserverID));
  ...
  const serverCallParamOverride = React.useMemo(() => {
    ...
    if (keyserverURL === ashoatKeyserverUrl) {
      return {
        keyserverInfos: {
          [(ashoatKeyserverID: string)]: {
  ...
  }, [keyserverURL, ashoatKeyserverUrl]);
