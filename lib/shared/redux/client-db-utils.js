...
function createUpdateDBOpsForThreadStoreThreadInfos(
  ...
  const replaceThreadOperations = updatedKeyedRawThreadInfosArray.map(thread =>
    createReplaceThreadOperation(thread.id, thread),
  );
...
function convertStoreOperationsToClientDBStoreOperations(
  ...
  return {
    ...
    holderStoreOperations: convertedHolderStoreOperations,
  };
}
