...
async function fetchAndProcessConversation(
  ...
  fetchUsersByFIDs: (fids: Array<string>) => Promise<$ReadOnlyMap<string, any>>,
  ...
  return React.useCallback(
    async (farcasterMessage: FarcasterMessage) => {
      ...
      if (
        ...
        viewerID !== rawMessageInfos[0].creatorID
      ) {
    ...
    [dispatch, fetchUsersByFIDs, viewerID],
  );
}
...
function useFarcasterSync(onComplete?: () => void): {
  ...
  const handleProgress = React.useCallback(
    (completed: number, total: number) => {
      const progressInfo = { completed, total };
