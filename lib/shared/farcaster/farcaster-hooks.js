...
async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      const errorIsAlwaysRetryable = alwaysRetryableErrors.some(
        alwaysRetryableError => message?.includes(alwaysRetryableError),
      );
      if (errorIsAlwaysRetryable) {
        if (addLog && operationName) {
          addLog(
            `Farcaster: Retry attempt ${attempt}/${maxRetries + 1} ignored for ${operationName}`,
            JSON.stringify({
              attempt,
              maxRetries,
              error: message,
        ...
        attempt--;
        // Extra sleep since it can take some time to reconnect to Tunnelbroker
        await sleep(3000);
      } else if (addLog && operationName) {
        addLog(
          `Farcaster: Retry attempt ${attempt}/${maxRetries + 1} failed for ${operationName}`,
          JSON.stringify({
            attempt,
...
function useFetchMessagesForConversation(): (
  ...
  return React.useCallback(
    async (
      ...
      const allFarcasterMessages: Array<FarcasterMessage> = [];
      ...
      try {
        ...
        do {
          ...
          if (messagesResult) {
            ...
            allFarcasterMessages.push(...farcasterMessages);
      ...
      const allUserFIDs = allFarcasterMessages.flatMap(message =>
        extractFarcasterIDsFromPayload(farcasterMessageValidator, message),
      );

      const fcUserInfos = await fetchUsersByFIDs(allUserFIDs);

      const result = allFarcasterMessages.flatMap(message =>
        convertFarcasterMessageToCommMessages(message, fcUserInfos, addLog),
      );

      if (result.length < allFarcasterMessages.length) {
        addLog(
          'Farcaster: Some messages failed to convert',
          JSON.stringify({
            conversationID,
            farcasterMessages: allFarcasterMessages.length,
            convertedMessages: result.length,
          }),
          new Set([logTypes.FARCASTER]),
        );
      }

      const userIDs = Array.from(fcUserInfos.entries()).map(
    ...
    async (farcasterMessage: FarcasterMessage) => {
      ...
      if (
        ...
        if (farcasterDCsLoaded) {
          ...
          // If DCs aren't fully synced yet, there's a possibility that this
          // message will be skipped if we don't process it now. We fetch the
          // conversation so the reducers don't immediately filter the message
          // out, and then we proceed to process this particular message
