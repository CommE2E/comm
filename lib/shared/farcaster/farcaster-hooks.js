...
const MAX_BATCH_MESSAGE_COUNT = 1000;
...
class BatchedUpdates {
  ...
  addUpdateInfo(updateInfo: ClientUpdateInfo): void {
    ...
    if (
      ...
      this.messageCount += updateInfo.rawMessageInfos.length;
    }
  }
  ...
  addMessageInfos(messageInfos: Array<RawMessageInfo>): void {
    ...
    this.messageCount += messageInfos.length;
  }
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    for (const itemResult of batchResults) {
      if (itemResult) {
        ...
        const messagesCount = batchedUpdates.getMessageCount();

        if (messagesCount > MAX_BATCH_MESSAGE_COUNT) {
          dispatch({
  ...
  return React.useCallback(
    async (
      ...
      try {
        ...
        if (unknownConversationIds.length > 0) {
          await processInBatchesWithReduxBatching(
        ...
        if (updateInfos.length > 0) {
          addLog(
            'Farcaster: Dispatching unread status updates',
            JSON.stringify({ updateCount: updateInfos.length }),
            new Set([logTypes.FARCASTER]),
          );
