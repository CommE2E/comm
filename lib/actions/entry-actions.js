...
import { getNextLocalID } from '../shared/message-utils.js';
import {
...
export type UseCreateEntryInput = $ReadOnly<
  | {
      +thick: false,
...
function useCreateEntry(): (
  ...
  return React.useCallback(
    async (input: UseCreateEntryInput) => {
      if (!input.thick) {
...
export type UseSaveEntryInput = $ReadOnly<
  | {
      +thick: false,
  ...
  return React.useCallback(
    async (input: UseSaveEntryInput) => {
      ...
      const op: DMEditEntryOperation = {
        ...
        lastUpdatedTime: saveEntryInfo.timestamp,
