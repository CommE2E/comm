...
const sendTextMessage =
  ...
  async input => {
    ...
    invariant(
      resultInterface,
      'getResultInfo not called before callKeyserverEndpoint resolves',
    );
    return {
...
// this has to be kept in sync with the constant used in
// SQLiteQueryExecutor::searchMessages
const numSearchMessagesPerPage = 20;
...
function useSearchMessages(): (
  ...
  return React.useCallback(
    async (input: SearchMessagesRequest) => {
      ...
      if (messages.length < numSearchMessagesPerPage) {
        return {
          endReached: true,
          messages,
        };
      }
      return { endReached: false, messages };
    },
    [thinThreadCallback],
  );
}
