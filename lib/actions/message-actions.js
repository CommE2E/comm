...
const fetchSingleMostRecentMessagesFromThreads =
  ...
  async threadIDs => {
    ...
    for (const keyserverID in sortedThreadIDs) {
      cursors[keyserverID] = Object.fromEntries(
    ...
    for (const keyserverID of allKeyserverIDs) {
      if (cursors[keyserverID] !== undefined) {
        requests[keyserverID] = {
          cursors: cursors[keyserverID],
          numberPerThread: 1,
        };
      }
    }
...
const toggleMessagePin =
  ...
  async input => {
    ...
    const response = (
      await callKeyserverEndpoint('toggle_message_pin', requests)
    )[keyserverID];
    return {
