...
import { createReplaceHoldersOperation } from '../ops/holder-store-ops.js';
import {
  type AuthMetadata,
...
import { holderStatuses, shouldHolderExist } from '../types/holder-types.js';
...
async function performBlobServiceHolderActions(
  ...
  handleInvalidCSAT?: (source: string) => Promise<void>,
  ...
  if (response.result === 'error') {
    ...
    void handleInvalidCSAT?.(`perform_blob_${action}`);
    return { succeeded: [], failed: inputs };
  }
      ...
      const holdersToAdd = ops
        .map(({ blobHash, type }) => {
          ...
          if (
            ...
            status === holderStatuses.PENDING_ESTABLISHMENT
          ) {
            return null;
          }
          return {
...
async function generateOpsToEstablishHoldersOnDevice(
  storedHolders: StoredHolders,
): Promise<$ReadOnlyArray<HolderStoreOperation>> {
  const deviceID = await getContentSigningKey();
  const newHolders: Array<HolderItem> = [];
  const hashesToRemove: Array<string> = [];
  for (const hash in storedHolders) {
    const { status } = storedHolders[hash];
    if (shouldHolderExist(status)) {
      newHolders.push({
        hash,
        holder: generateBlobHolder(deviceID),
        // This will trigger HoldersHandler, which is responsible for making
        // sure the holder will be established.
        status: holderStatuses.NOT_ESTABLISHED,
      });
    } else {
  ...
  return [
    { type: 'remove_holders', payload: { hashes: hashesToRemove } },
    createReplaceHoldersOperation(newHolders),
  ];
}
...
export {
  ...
  generateOpsToEstablishHoldersOnDevice,
};
