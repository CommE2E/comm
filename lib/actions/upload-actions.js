...
export type MediaMetadataUploadInput = {
  +blobHash: string,
  +encryptionKey: string,
  +mimeType: string,
  +dimensions: ?Dimensions,
...
export type MediaMetadataUploadAction = (input: {
  +uploadInput: MediaMetadataUploadInput,
  +keyserverOrThreadID: string,
}) => Promise<UploadMultimediaResult>;

const mediaMetadataUpload =
  (
    callKeyserverEndpoint: CallKeyserverEndpoint,
    authMetadata: AuthMetadata,
  ): MediaMetadataUploadAction =>
  async input => {
    const { uploadInput, keyserverOrThreadID } = input;
    const { encryptionKey, loop, dimensions, thumbHash, filename, mimeType } =
      uploadInput;
...
function useMediaMetadataUpload(): MediaMetadataUploadAction {
  const identityContext = React.useContext(IdentityClientContext);
  invariant(identityContext, 'Identity context should be set');
  const { getAuthMetadata } = identityContext;
  ...
  const blobUploadAction = React.useCallback(
    (
      ...
      async input => {
        const authMetadata = await getAuthMetadata();
        const authenticatedUploadAction = mediaMetadataUpload(
          callSingleKeyserverEndpoint,
          authMetadata,
        );
        return authenticatedUploadAction(input);
