...
function usePrimaryDeviceLogOut(): () => Promise<LogOutResult> {
  ...
  const broadcastOlmMessage = useBroadcastOlmMessage();
  ...
  return React.useCallback(async () => {
    ...
    await broadcastOlmMessage(JSON.stringify(messageContents), recipients);
...
function useSecondaryDeviceLogOut(): () => Promise<LogOutResult> {
  ...
  const sendOlmMessage = useBroadcastOlmMessage();
  ...
  return React.useCallback(async () => {
    ...
    await sendOlmMessage(JSON.stringify(messageContents), [recipient]);

    // log out of identity service, keyserver and visually
    return logOut();
  }, [identityContext, sendOlmMessage, logOut]);
}
