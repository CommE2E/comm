...
function useIdentityPasswordRegisterReserved(): (
  usename: string,
  password: string,
  ...
  const identityClient = client?.identityClient;
  invariant(identityClient, 'Identity client should be set');
  if (!identityClient.registerReservedPasswordUser) {
    throw new Error('Register password user method unimplemented');
  }
  return identityClient.registerReservedPasswordUser;
}
...
function useIdentityWalletRegisterReserved(): (
  walletAddress: string,
  siweMessage: string,
  siweSignature: string,
  keyserverMessage: string,
  ...
  const identityClient = client?.identityClient;
  invariant(identityClient, 'Identity client should be set');
  if (!identityClient.registerReservedWalletUser) {
    throw new Error('Register wallet user method unimplemented');
  }
  return identityClient.registerReservedWalletUser;
}
...
function useIdentityPasswordLogIn(): (
  ...
  const registerReservedPasswordUser = useIdentityPasswordRegisterReserved();
    ...
    (username, password) => {
      ...
      return (async () => {
        ...
        try {
          ...
          if (
            identityClient.registerReservedPasswordUser &&
...
function useIdentityWalletLogIn(): (
  ...
  const identityClient = client?.identityClient;
  const callClaimUsername = useClaimUsername();
  const registerReservedWalletUser = useIdentityWalletRegisterReserved();

  invariant(identityClient, 'Identity client should be set');
  return React.useCallback(
    (walletAddress, siweMessage, siweSignature) => {
      if (!identityClient) {
        throw new Error('Identity service client is not initialized');
      }
      return (async () => {
        try {
          return await identityClient.logInWalletUser(
            walletAddress,
            siweMessage,
            siweSignature,
          );
          ...
          if (
            identityClient.registerReservedWalletUser &&
            getMessageForException(e) ===
              'need keyserver message to claim username'
          ) {
            const { message, signature } = await callClaimUsername();
            return await registerReservedWalletUser(
              walletAddress,
              siweMessage,
              siweSignature,
              message,
    ...
    [identityClient, callClaimUsername, registerReservedWalletUser],
  );
}
...
export {
  ...
  useIdentityPasswordRegisterReserved,
