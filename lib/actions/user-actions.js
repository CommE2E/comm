...
function useBaseLogOut(
  ...
  return React.useCallback(
    async (
      ...
      const logoutSource = `base_logout_${JSON.stringify(options)}`;
      ...
      const identityPromise = (async () => {
        ...
        if (logOutType === 'primary_device') {
          ...
          callIdentityClientLogOut = async () =>
            identityClient.logOutSecondaryDevice(preRequestAuthMetadata);
        } else {
          callIdentityClientLogOut = async () => {
            try {
              await identityClient.logOut(logoutSource, preRequestAuthMetadata);
            } catch (e) {
              ...
              // old flow
              try {
                await sendLogoutMessage(preRequestAuthMetadata);
              } catch (err) {
...
function useLogOut(): (
  ...
  return React.useCallback(
    async preRequestAuthMetadata => {
      if (usingRestoreFlow) {
        const isPrimaryDevice = await checkIfPrimaryDevice(
          preRequestAuthMetadata,
        );
        return isPrimaryDevice
          ? callPrimaryDeviceLogOut(preRequestAuthMetadata)
          : callSecondaryDeviceLogOut(preRequestAuthMetadata);
      } else {
        return callLegacyLogOut(preRequestAuthMetadata);
      }
    },
    [
      callLegacyLogOut,
...
function useIdentityLogOut(
  ...
  return React.useCallback(async () => {
    ...
    const description = `identity ${logOutType} log_out`;
    ...
    if (logOutType === 'primary_device') {
      ...
      callIdentityClientLogOut =
        identityClient.logOutPrimaryDevice?.(description);
    } else if (logOutType === 'secondary_device') {
      callIdentityClientLogOut = identityClient.logOutSecondaryDevice();
    } else {
      callIdentityClientLogOut = identityClient.logOut(description);
    }
    try {
      await promiseWithTimeout(
        callIdentityClientLogOut,
...
function useInvalidCSATLogOut(): (source: string) => Promise<void> {
  ...
  return React.useCallback(
    (source: string) => {
      ...
      if (isStaffRelease) {
        showAlert('InvalidCSATLogOut', source);
      }
      ...
      promiseRef.current = dispatchActionPromise(
        logOutActionTypes,
        keyserverLogOutPromise(),
      );
      return promiseRef.current;
    },
    [
...
function usePrimaryDeviceLogOut(): (
  preRequestAuthMetadata?: AuthMetadata,
) => Promise<LogOutResult> {
  ...
  return React.useCallback(
    async preRequestAuthMetadata => {
      ...
      // device list.
      // We're relying on Tunnelbroker session stil existing after calling
      // logout and auth metadata not yet cleared at this point.
      const logOutResult = await logOut(preRequestAuthMetadata);
...
): (preRequestAuthMetadata?: AuthMetadata) => Promise<void> {
  const identityContext = React.useContext(IdentityClientContext);
  if (!identityContext) {
    throw new Error('Identity service client is not initialized');
  }
...
function useSecondaryDeviceLogOut(): (
  preRequestAuthMetadata?: AuthMetadata,
) => Promise<LogOutResult> {
  ...
  return React.useCallback(
    async preRequestAuthMetadata => {
      try {
        await promiseWithTimeout(
          sendLogoutMessage(preRequestAuthMetadata),
          logoutTimeout,
          'send logout message',
        );
      } catch (e) {
      ...
      return logOut(preRequestAuthMetadata);
    },
...
function useIdentityPasswordRegister(): (
  ...
  return React.useCallback(
    async (
      ...
      const response = await registerPasswordUser(
        username,
        password,
        fid,
        farcasterDCsToken,
...
export type UseUpdateSubscriptionInput = $ReadOnly<{
...
const processNewUserIDsActionType = 'PROCESS_NEW_USER_IDS' as const;
...
export type RestoreProtocolResult = $ReadOnly<{
  ...IdentityAuthResult,
