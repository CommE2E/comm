...
async function logOut(
  ...
  return response.cookie_change.calendar_infos;
}
...
async function logInAndFetchEntries(
  ...
  const entryQuery = currentInnerEntryQuery();
  const response = await fetchJSON('login.php', {
    username,
    password,
    'inner_entry_query': entryQuery,
  });
  return {
    calendarInfos: response.cookie_change.calendar_infos,
    userInfo: {
    ...
    entriesResult: {
      startDate: entryQuery.start_date,
      endDate: entryQuery.end_date,
