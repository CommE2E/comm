...
const claimUsername =
  ...
  async () => {
    ...
    const response = (
      await callKeyserverEndpoint('claim_username', requests, {
...
const logIn =
  ...
  async logInInfo => {
    ...
    const { logInActionSource, calendarQuery, ...restLogInInfo } = logInInfo;

    // Eventually the list of keyservers will be fetched from the
    // identity service
    const keyserverIDs = [ashoatKeyserverID];
...
const updateSubscription =
  ...
  async input => {
    ...
    const response = (
      await callKeyserverEndpoint('update_user_subscription', requests)
    )[keyserverID];
    return {
