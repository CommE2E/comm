...
const identityServiceOlmAuth =
  (
    ...
    baseIdentityServiceOlmAuthPayload: BaseIdentityServiceOlmAuthRequest,
    options?: ?CallServerEndpointOptions,
  ) => Promise<string>) =>
  async (baseIdentityServiceOlmAuthPayload, options) => {
    const watchedIDs = threadWatcher.getWatchedIDs();
    const response = await callServerEndpoint(
      'identity_service_olm_auth',
      {
        ...baseIdentityServiceOlmAuthPayload,
        watchedIDs,
        platformDetails: getConfig().platformDetails,
      },
      {
...
export {
  ...
  identityServiceOlmAuth,
  identityServiceOlmAuthActionTypes,
};
