...
} from '../types/tunnelbroker/peer-to-peer-message-types';
...
function useSecondaryDeviceLogOut(): () => Promise<LogOutResult> {
  ...
  const performLogOut: () => Promise<LogOutResult> =
    React.useCallback(async () => {
      const { userID, deviceID } = await getAuthMetadata();
      if (!deviceID || !userID) {
        console.log('no auth metadata');
      ...
      // create and send Olm Tunnelbroker message
      ...
      return logOut();
    }, [identityClient, getAuthMetadata, sendMessage, logOut]);
...
function useIdentitySecondaryDeviceLogIn(): (
  ...
  const { signMessage } = getConfig().olmAPI;

  return React.useCallback(
    async (userID: string) => {
      const nonce = await generateNonce();
      const nonceSignature = await signMessage(nonce);
      return await uploadKeysForRegisteredDeviceAndLogIn(userID, {
        nonce,
        nonceSignature,
      });
    },
    [generateNonce, signMessage, uploadKeysForRegisteredDeviceAndLogIn],
  );
}
