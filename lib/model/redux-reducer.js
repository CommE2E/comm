...
function reduceLoadingStatuses<T: BaseAppState>(
  ...
  if (
    action.type === "FETCH_MONTH_ENTRIES_STARTED" ||
      action.type === "FETCH_ALL_DAY_ENTRIES_STARTED"
  ) {
    const keyName = action.type.substring(0, action.type.length - 8);
    ...
    action.type === "FETCH_MONTH_ENTRIES_FAILED" ||
      action.type === "FETCH_ALL_DAY_ENTRIES_FAILED"
  ) {
    const keyName = action.type.substring(0, action.type.length - 7);
