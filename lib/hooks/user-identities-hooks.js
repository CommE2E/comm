...
function useGetCommFCUsersForFIDs(): (
  farcasterIDs: $ReadOnlyArray<string>,
) => Promise<$ReadOnlyMap<string, FarcasterUser | void>> {
  const auxUserInfos = useSelector(state => state.auxUserStore.auxUserInfos);
  ...
  return React.useCallback(
    async (farcasterIDs: $ReadOnlyArray<string>) => {
      const fcUsers = new Map<string, FarcasterUser | void>();
      const unresolvedFIDs = new Set(farcasterIDs);
