  ...
  return React.useCallback(
    async (userIDs: $ReadOnlyArray<string>) => {
      ...
      for (const userID of userIDs) {
        ...
        } else if (!peerDeviceList || peerDeviceList.length > 0) {
      ...
      if (usersNeedingFetch.length > 0) {
        ...
        for (const userID of usersNeedingFetch) {
          ...
          if (identities[userID] && peerDeviceList.length > 0) {
...
function useUsersSupportingProtocols(users: $ReadOnlyArray<AccountUserInfo>): {
  ...
  const [protocolSupport, setProtocolSupport] = React.useState({
    allUsersSupportThickThreads: false,
    allUsersSupportFarcasterThreads: false,
  });
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      setProtocolSupport({
        allUsersSupportThickThreads: users.every(userInfo =>
          usersSupportingThickThreads.get(userInfo.id),
        ),
        allUsersSupportFarcasterThreads: users.every(userInfo =>
          usersSupportingFarcasterThreads.get(userInfo.id),
        ),
      });
    })();
