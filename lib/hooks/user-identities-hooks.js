...
export type UsersSupportThickThreads = (
  userIDs: $ReadOnlyArray<string>,
) => Promise<$ReadOnlyMap<string, boolean | void>>;

function useUsersSupportThickThreads(): UsersSupportThickThreads {
  ...
  return React.useCallback(
    async (userIDs: $ReadOnlyArray<string>) => {
      ...
      for (const userID of userIDs) {
        if (userID === '###7A8BC7B3-BBBE-4980-8737-E3E67A26FBD2') {
          usersSupportingThickThreads.set(userID, false);
        } else if (auxUserInfos[userID]?.deviceList) {
  ...
  return React.useCallback(
    async (userIDs: $ReadOnlyArray<string>) => {
      ...
      for (const userID of userIDs) {
        ...
        if (
          userIDIsFarcasterFID ||
          auxUserInfos[userID]?.supportsFarcasterDCs
        ) {
