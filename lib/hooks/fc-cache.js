...
function useFarcasterAvatar(fid: ?string): ?string {
  ...
  const [farcasterAvatars, setFarcasterAvatars] = React.useState<
    $ReadOnlyMap<string, string>,
  >(new Map());

  React.useEffect(() => {
    if (!fcCache || !fid || cachedAvatar !== undefined) {
      return;
    }
    void (async () => {
      ...
      setFarcasterAvatars(oldFarcasterAvatars => {
        const newFarcasterAvatars = new Map(oldFarcasterAvatars);
        newFarcasterAvatars.set(fid, avatarURL);
        return newFarcasterAvatars;
      });
    })();
  ...
  return React.useMemo(() => {
    if (!fid) {
      ...
      return farcasterAvatars.get(fid);
    }
  }, [fid, cachedAvatar, farcasterAvatars]);
}
