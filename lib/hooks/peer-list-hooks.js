...
function useBroadcastAccountDeletion(options: {
  ...
  const peers = useSelector(getPeersUserAndDeviceIDs);
  const broadcastMessage = useBroadcastOlmMessage();
  ...
  return React.useCallback(async () => {
    ...
    await broadcastMessage(rawPayload, recipients);
  }, [broadcastMessage, getAuthMetadata, includeOwnDevices, peers]);
}
