...
function usePlaintextMediaUpload(): PlaintextMediaUploadAction {
  const identityContext = React.useContext(IdentityClientContext);
  invariant(identityContext, 'Identity context should be set');
  const { getAuthMetadata } = identityContext;
  ...
  return React.useCallback(
    async (input): Promise<UploadMultimediaResult> => {
      ...
      let mediaUploadCallback = plaintextMediaUploadHandler;
      if (callbacks && callbacks.plaintextMediaUploadHandler) {
        mediaUploadCallback = callbacks.plaintextMediaUploadHandler;
      }
      ...
      try {
        ...
        if (!dimensions) {
          throw new Error('dimensions are required for plaintext uploads');
        }
        return {
          id: response.mediaID,
          uri: makeBlobServiceEndpointURL(blobService.httpEndpoints.GET_MEDIA, {
