...
type LoginFlow = 'restore' | 'secondary_device' | 'legacy';
...
function useLogIn(
  ...
  let logOutType;
  if (loginFlow === 'restore') {
    logOutType = 'primary_device';
  } else if (loginFlow === 'secondary_device') {
    logOutType = 'secondary_device';
  } else {
    logOutType = 'legacy';
  }
  const identityLogOut = useIdentityLogOut(logOutType);
...
function useSecondaryDeviceLogIn(): (userID: string) => Promise<void> {
  ...
  const logIn = useLogIn('secondary_device');
  return React.useCallback(
