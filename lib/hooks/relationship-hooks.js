...
function useUpdateRelationships(): (
  ...
  const isSendingRobotextToThickThread = React.useRef<Map<string, boolean>>(
    new Map(),
  );
  ...
  React.useEffect(() => {
    ...
    for (const step of steps) {
      const { userID, thickThreadID, relationshipOperation, resolve } = step;
      if (!threadInfos[thickThreadID]) {
        newSteps.push(step);
        continue;
      }
      if (isSendingRobotextToThickThread.current.get(thickThreadID)) {
        continue;
      }
      isSendingRobotextToThickThread.current.set(thickThreadID, true);
      void (async () => {
        await sendRobotextToThickThread(
          userID,
          thickThreadID,
  ...
  }, [sendRobotextToThickThread, steps, threadInfos]);
