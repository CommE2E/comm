...
function useChangeThreadSettings(): (
  ...
  const updateFarcasterGroupNameAndDescription =
    useUpdateFarcasterGroupNameAndDescription();
  const updateFarcasterThreadAvatar = useUpdateFarcasterThreadAvatar();
...
const removeUsersFromThread =
  ...
  async input => {
    const { threadInfo, memberIDs } = input;
...
function useRemoveUsersFromThread(): (
  ...
  const keyserverCall = useKeyserverCall(removeUsersFromThread);
  const dispatchActionPromise = useDispatchActionPromise();
  const modifyFarcasterMembership = useModifyFarcasterMembershipInput();
  const fetchConversation = useRefreshFarcasterConversation();
  const auxUserStore = useSelector(state => state.auxUserStore);
  return React.useCallback(
    async (input: RemoveUsersFromThreadInput) => {
      const { threadInfo, memberIDs } = input;
      const protocol = threadSpecs[threadInfo.type].protocol();
      invariant(
        protocol.removeUsersFromThread,
        `removeUsersFromThread not supported for thread type ` +
          `${threadInfo.type}`,
      );

      const customKeyName =
        memberIDs.length > 0
          ? `${removeUsersFromThreadActionTypes.started}:${memberIDs[0]}`
          : undefined;

      return protocol.removeUsersFromThread(
        {
          threadInfo,
          memberIDs,
          customKeyName,
...
function useChangeThreadMemberRoles(): (
  ...
  return React.useCallback(
    async (input: ChangeThreadMemberRolesInput) => {
      ...
      const protocol = threadSpecs[threadInfo.type].protocol();
      invariant(
        protocol.changeThreadMemberRoles,
        `changeThreadMemberRoles not supported for thread type ` +
          `${threadInfo.type}`,
      );
      return protocol.changeThreadMemberRoles(
        {
          threadInfo,
          memberIDs,
          newRole,
