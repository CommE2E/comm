...
async function migrateMessageMediaToKeyserver(
  ...
  const processMediaResult = (
    ...
    messageMedia.push(newMedia);

    if (!update) {
      return;
    }

    const { blobsToRemoveHolder, ...mediaUpdate } = update;
  ...
  const reassignmentPromises = messageInfo.media.map(async media => {
    if (
      ...
      return processMediaResult(media.id, media);
    }
    ...
    if (media.type !== 'encrypted_video') {
      ...
      return processMediaResult(media.id, updatedMedia, mediaChanges);
    }
