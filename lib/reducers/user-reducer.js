...
function reduceUserInfos(
  ...
  if (
    action.type === processNewUserIDsActionType ||
    action.type === processFarcasterOpsActionType
  ) {
    const userIDs =
      action.type === processNewUserIDsActionType
        ? action.payload.userIDs
        : action.payload.userIDs || [];
    const filteredUserIDs = userIDs.filter(id => !state.userInfos[id]);
    if (filteredUserIDs.length === 0) {
      return [state, [], []];
    }
    const newUserInfosArray = filteredUserIDs.map(id => {
      const avatar = extractFIDFromUserID(id) ? { type: 'farcaster' } : null;
      return {
        id,
        username: null,
        avatar,
