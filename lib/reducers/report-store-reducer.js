...
import { assignReportsID, isReportEnabled } from '../utils/report-utils.js';
...
export default function reduceReportStore(
  ...
  const newInconsistenciesWithID = assignReportsID(newInconsistencies);
  const updatedReports =
    newInconsistenciesWithID.length > 0
      ? [...state.queuedReports, ...newInconsistenciesWithID].filter(report =>
  ...
  if (action.type === updateReportsEnabledActionType) {
    ...
    const reports = assignReportsID(action.payload.reports);
