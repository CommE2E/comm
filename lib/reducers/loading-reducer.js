...
const logOutActionsPrefixSet: Set<string> = new Set(
  Array.from(logOutSuccessActionsSet, actionName => {
...
function reduceLoadingStatuses(
  ...
  if (logInSuccessActionsSet.has(action.type)) {
    const newState = { ...state };
    for (const key in state) {
      if (logOutActionsPrefixSet.has(key)) {
        newState[key] = {};
      }
    }
    state = newState;
