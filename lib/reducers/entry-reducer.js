...
import { setCurLocalID } from '../actions/entry-actions';
...
function reduceEntryInfos(
  ...
  const lastUserInteractionCalendar = inputLastUserInteractionCalendar;
  if (
    ...
    const newEntryInfos = _pickBy(
      (entry: EntryInfo) => authorizedCalendarInfos[entry.threadID],
    )(entryInfos);
    ...
    if (action.payload.localID) {
      ...
      if (entryInfos[serverID]) {
        ...
        return [entryInfos, daysToEntries, lastUserInteractionCalendar];
      }
    ...
    return [newEntryInfos, daysToEntries, lastUserInteractionCalendar];
    ...
    for (let entryKey in action.payload.entryInfos) {
      const localID = action.payload.entryInfos[entryKey].localID;
      if (localID) {
    ...
    setCurLocalID(highestLocalIDFound + 1);
  }
  return [entryInfos, daysToEntries, lastUserInteractionCalendar];
}
