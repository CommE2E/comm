...
function reduceEntryInfos(
  ...
  if (
    ...
    const calendarResult = payload.calendarResult;
    const rawEntryInfos = mergeUpdateEntryInfos(
      calendarResult.rawEntryInfos,
      payload.updatesResult.newUpdates,
    );
    const [updatedEntryInfos, updatedDaysToEntries] = mergeNewEntryInfos(
      entryInfos,
      daysToEntries,
    ...
    return {
      ...
      actualizedCalendarQuery,
      lastUserInteractionCalendar,
      inconsistencyResponses: inconsistencyResponses.filter(
        response =>
