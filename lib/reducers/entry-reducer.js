...
function reduceEntryInfos(
  ...
  if (
    ...
    const newDaysToEntries = daysToEntriesFromEntryInfos(
      _values(pollEntryInfos),
    );

    const pushNewEntryInfos = mergeUpdateEntryInfos(
      [],
      action.payload.updatesResult.viewerUpdates,
    );
    const [pushEntryInfos] = mergeNewEntryInfos(
      rekeyedEntryInfos,
      pushNewEntryInfos,
      newThreadInfos,
    );
    ...
    if (payload.type === pingResponseTypes.FULL) {
      ...
      ({ calendarQuery } = payload);
    }

    const { updatesResult, deltaEntryInfos } = payload;
    let pushResult;
    if (deltaEntryInfos) {
      const pushNewEntryInfos = mergeUpdateEntryInfos(
        deltaEntryInfos,
    ...
    let newInconsistencies = [];
    if (pollResult && pushResult) {
      const [ pollEntryInfos ] = pollResult;
      const [ pushEntryInfos ] = pushResult;
    ...
    let updatedEntryInfos, updatedDaysToEntries;
    if (payload.type === pingResponseTypes.FULL) {
      invariant(pollResult, "should be set");
      [ updatedEntryInfos, updatedDaysToEntries ] = pollResult;
    } else {
      invariant(pushResult, "should be set");
      [ updatedEntryInfos, updatedDaysToEntries ] = pushResult;
    }
...
function findInconsistencies(
  ...
  calendarQuery: CalendarQuery,
): EntryPollPushInconsistencyClientResponse[] {
  ...
  if (action.type === pingActionTypes.success) {
    if (action.payload.type === pingResponseTypes.FULL) {
      ...
      // EntryPollPushInconsistencyClientResponse in this one
      action = {
        type: "PING_SUCCESS",
  ...
  return [{
    type: serverRequestTypes.ENTRY_POLL_PUSH_INCONSISTENCY,
