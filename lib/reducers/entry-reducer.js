...
function reduceEntryInfos(
  ...
  if (
    ...
    const { calendarResult, updatesResult, deltaEntryInfos } = payload;
    const { calendarQuery, rawEntryInfos: pullNewEntryInfos } = calendarResult;

    const [ pollEntryInfos, updatedDaysToEntries ] = mergeNewEntryInfos(
      entryInfos,
      pullNewEntryInfos,
      newThreadInfos,
      {
...
function findInconsistencies(
  ...
  // TODO: we should call findInconsistencies from the 4 entry endpoints as well
