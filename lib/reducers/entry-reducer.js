...
function mergeNewEntryInfosOps(
  ...
  for (const rawEntryInfo of newEntryInfos) {
    ...
    if (_isEqual(currentEntryInfo)(newEntryInfo)) {
      ...
      ops.push(
        createReplaceEntryOperation(serverID, newEntryInfo, threadInfos),
      );
  ...
  if (
    ...
    const ops = [
      ...
      ...Object.entries(newEntryInfos).map(([id, entry]) =>
        createReplaceEntryOperation(id, entry, newThreadInfos),
      ),
    ];
    return {
      entryStore: entryStoreOpsHandlers.processStoreOperations(entryStore, ops),
    ...
    const ops = [
      createReplaceEntryOperation(localID, entryInfo, newThreadInfos),
    ];
    return {
      entryStore: {
        ...entryStoreOpsHandlers.processStoreOperations(entryStore, ops),
    ...
    const newEntry: RawEntryInfo = {
      ...entryInfos[payload.id],
      text: payload.dbText,
    };
    const ops = [
      createReplaceEntryOperation(payload.id, newEntry, newThreadInfos),
    ];
    return {
      entryStore: entryStoreOpsHandlers.processStoreOperations(entryStore, ops),
      entryStoreOperations: ops,
    ...
    const ops = [createReplaceEntryOperation(id, newEntry, newThreadInfos)];
    return {
      entryStore: {
        ...entryStoreOpsHandlers.processStoreOperations(entryStore, ops),
        lastUserInteractionCalendar: Date.now(),
...
function markDeletedEntries(
  ...
  for (const deletedEntryID of deletedEntryIDs) {
    ...
    ops.push(
      createReplaceEntryOperation(deletedEntryID, newEntry, threadInfos),
    );
  }
  return ops;
}
