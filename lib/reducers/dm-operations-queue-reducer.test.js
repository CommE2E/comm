...
describe('reduceDMOperationsQueue', () => {
  ...
  describe('reportUnshimmingOperationCompletedActionType', () => {
    it('should remove shimmed operation by ID', () => {
      const store: QueuedDMOperations = {
        threadQueue: {},
        messageQueue: {},
        entryQueue: {},
      ...
      expect(result.store.shimmedOperations).toHaveLength(1);
      expect(result.store.shimmedOperations[0].id).toBe('op2');
    });
  });
});
