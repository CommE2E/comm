...
describe('DB ops reducer', () => {
  const store: DBOpsStore = {
    queuedOps: [
      { ops: emptyOps, messageData: { messageID: '5', senderDeviceID: '' } },
      { ops: emptyOps },
      { ops: null, messageData: { messageID: '7', senderDeviceID: '' } },
    ],
  };
  ...
  it('should remove the first entry', () => {
    ...
    expect(newState.queuedOps[1].messageData?.messageID).toEqual('7');
  });
});
