...
const loggedActionTypes = [
  ...Object.values(createUserDataBackupActionTypes),
  ...Object.values(createUserKeysBackupActionTypes),
  ...Object.values(restoreUserDataStepActionTypes),
];
...
export default function baseReducer<N: BaseNavInfo, T: BaseAppState<N>>(
  ...
  const onStateDifferenceForStaff = (message: string) => {
    ...
    if (isCurrentUserStaff || isDev) {
      getConfig().showAlert('State difference found', message);
    }
  };
  ...
  return {
    state: {
      ...
      restoreBackupState: (() => {
        ...
        const shouldLog = loggedActionTypes.includes(type);
        if (shouldLog) {
          console.log(`[Redux] Action ${type}:\t\t`, { payload });
        }
        const result = reduceRestoreBackupState(
          state.restoreBackupState,
          action,
        );
        if (shouldLog) {
          console.log(`[Redux] State after ${type}:\t`, result);
        }
        return result;
      })(),
    },
