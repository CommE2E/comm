...
export default function reduceKeyserverStore(
  ...
  if (action.type === addKeyserverActionType) {
    const { keyserverAdminUserID, newKeyserverInfo } = action.payload;
    if (state.keyserverInfos[keyserverAdminUserID]) {
      return {
    ...
    const newKeyserverInfos = action.payload.keyserverInfos;

    if (!newKeyserverInfos) {
      return {
        keyserverStore: state,
        keyserverStoreOperations: [],
      };
    ...
    const newKeyserverStore: KeyserverStore = {
      ...state,
      keyserverInfos: newKeyserverInfos,
    };

    return {
      keyserverStore: newKeyserverStore,
      keyserverStoreOperations: [],
    };
