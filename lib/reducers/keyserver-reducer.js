...
export default function reduceKeyserverStore(
  ...
  if (
    ...
    return {
      keyserverInfos: {
        [ashoatKeyserverID]: {
          ...
          urlPrefix: state.keyserverInfos[ashoatKeyserverID].urlPrefix,
    ...
    const filteredKeys = Object.keys(state.keyserverInfos).filter(
      key => key !== action.payload.keyserverAdminUserID,
    );

    const filteredKeyserverInfos = filteredKeys.reduce(
      (newKeyserverInfos, key) => {
        newKeyserverInfos[key] = state.keyserverInfos[key];
        return newKeyserverInfos;
      },
      {},
    );
    ...
    return {
      ...state,
      keyserverInfos: filteredKeyserverInfos,
    };
