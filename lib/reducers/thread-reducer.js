...
const PROCESSED_THREAD_STORE_INVARIANTS_DISABLED = !isDev;
...
export default function reduceThreadInfos(
  ...
  if (
    ...
    const updatedStore = {
      threadInfos: {
        ...state.threadInfos,
        [action.payload.threadID]: newThreadInfo,
      },
    };
    const threadStoreOperations = [
      {
    ...
    assertThreadStoreThreadsAreEqual(processedStore, updatedStore, action.type);
    return [updatedStore, newInconsistencies];
...
function assertThreadStoreThreadsAreEqual(
  processedThreadStore: ThreadStore,
  expectedThreadStore: ThreadStore,
  actionType: string,
) {
  if (PROCESSED_THREAD_STORE_INVARIANTS_DISABLED) {
    return;
  }
  const processedThreadStoreKeys = Object.keys(
    processedThreadStore.threadInfos,
  );
  const expectedThreadStoreKeys = Object.keys(expectedThreadStore.threadInfos);

  const inProcessedButNotExpected = _difference(processedThreadStoreKeys)(
    expectedThreadStoreKeys,
  );
  const inExpectedButNotProcessed = _difference(expectedThreadStoreKeys)(
    processedThreadStoreKeys,
  );

  invariant(
    _isEqual(processedThreadStore.threadInfos)(expectedThreadStore.threadInfos),
    `${actionType}: processedThreadStore.threadInfos should be equal` +
      ` to expectedThreadStore.threadInfos.` +
      ` ${inProcessedButNotExpected.toString()}` +
      ` are thread IDs in processed but not expected.` +
      ` ${inExpectedButNotProcessed.toString()}` +
      ` are thread IDs in expected but not processed.`,
  );
}
