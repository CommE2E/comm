...
function freshMessageStore(
  ...
  for (let threadID in threadInfos) {
    ...
    if (threadInfo.currentUserRole.isMember && !threads[threadID]) {
      threads[threadID] = {
        messageIDs: [],
...
function mergeNewMessages(
    ...
    _pickBy((messageIDs: string[], threadID: string) => {
      return !threadInfos ||
        (threadInfos[threadID] &&
          threadInfos[threadID].currentUserRole.isMember);
    }),
  ...
  for (let threadID in threadInfos) {
    ...
    if (threadInfo.currentUserRole.isMember && !thread) {
      threads[threadID] = {
        messageIDs: [],
        // We can conclude that startReached, since no messages were returned
        startReached: true,
...
function filterByNewThreadInfos(
  ...
  const memberThreadInfos =
    _pickBy('currentUserRole.isMember')(threadInfos);
