...
function freshMessageStore(
  ...
  const messageStoreOperations = [
    ...
    ...createReplaceMessageStoreThreadsOperations(threads, threadInfos),
...
function reassignMessagesToRealizedThreads(
  ...
  for (const storeMessageID in messageStore.messages) {
    ...
    // we don't want to reassign from panding threads with selected
    // protocols to different threads
...
function mergeNewMessages(
  ...
  const orderedNewMessageInfos = _flow(
    _map((messageInfo: RawMessageInfo) => {
      const inputID = messageID(messageInfo);
      invariant(
      ...
      if (
        ...
        } else if (messageInfo.id) {
          // For keyserver messages, if neither the serverID nor the localID
          // from the delivered RawMessageInfo exists in the client store, then
          // this message is brand new to us. Ignore any localID provided by
          // the server.
          // Failed messages that don't use a keyserver protocol have
          // localID and don't have an ID yet. We shouldn't remove their
          // localID to avoid a situation when a message doesn't have any
          // type of ID.
  ...
  const threads = _flow(
    _mapValuesWithKeys((messageIDs: string[], threadID: string) => {
      ...
      for (const id of oldNotInNew) {
        ...
        if (localInfo) {
          newMessageOps.push(
            createReplaceMessageStoreLocalOperation(id, localInfo),
          );
        }
      }
      const startReached =
  ...
  const dbOps = [
    ...newMessageOps,
    ...createReplaceMessageStoreThreadsOperations(filteredThreads, threadInfos),
  ];

  newMessageOps.push(
    ...createReplaceMessageStoreThreadsOperations(updatedThreads, threadInfos),
  );
...
function reduceMessageStore(
  ...
  if (action.type === legacyLogInActionTypes.success) {
    ...
    const messageStoreOperations = [
      createReplaceMessageStoreLocalOperation(localID, newLocalMessageInfo),
    ];
    ...
    for (const localMessageID in actionPayloadMessageStoreLocalMessageInfos) {
      if (
        ...
        messageStoreOperations.push(
          createReplaceMessageStoreLocalOperation(localMessageID, {
            ...actionPayloadMessageStoreLocalMessageInfos[localMessageID],
            sendFailed: true,
          }),
        );
    ...
    const localOperation: ReplaceMessageStoreLocalMessageInfoOperation =
      createReplaceMessageStoreLocalOperation(composableMessageID, {
    ...
    let operations = messageStoreOperations;
    if (additionalMessageInfos) {
      const { messageStoreOperations: additionalOperations } = mergeNewMessages(
        newMessageStore,
        additionalMessageInfos,
        messagesResult.truncationStatuses,
        newThreadInfos,
