...
function freshMessageStore(
  ...
  for (let threadID in threadInfos) {
    ...
    if (viewerIsMember(threadInfo) && !threads[threadID]) {
      threads[threadID] = {
        messageIDs: [],
...
function mergeNewMessages(
  ...
  const threads = _flow(
    _pickBy((messageIDs: string[], threadID: string) => {
      return !threadInfos ||
        (threadInfos[threadID] && viewerIsMember(threadInfos[threadID]));
    }),
  ...
  for (let threadID in threadInfos) {
    ...
    if (viewerIsMember(threadInfo) && !thread) {
      threads[threadID] = {
        messageIDs: [],
        // We can conclude that startReached, since no messages were returned
        startReached: true,
...
function filterByNewThreadInfos(
  ...
  const memberThreadInfos = _pickBy(viewerIsMember)(threadInfos);
  const messageIDsToRemove = [];
  for (let threadID in messageStore.threads) {
    if (memberThreadInfos[threadID]) {
      continue;
    }
  ...
  return {
    ...
    threads: _pick(Object.keys(memberThreadInfos))(messageStore.threads),
  };
}
...
function reduceMessageStore(
  ...
  if (
    ...
    action.type === "CHANGE_THREAD_SETTINGS_SUCCESS" ||
      ...
      action.type === "REMOVE_USERS_FROM_THREAD_SUCCESS" ||
      action.type === "CHANGE_THREAD_MEMBER_ROLES_SUCCESS"
  ) {
    return mergeNewMessages(
      messageStore,
