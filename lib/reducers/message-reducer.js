    ...
    if (message.type === messageTypes.IMAGES) {
      ...
      for (const singleMedia of message.media) {
        if (singleMedia.id !== currentMediaID) {
          ...
          // these fields are absent in encrypted_photo type
          const original = _omit(['uri', 'localMediaSelection'])(singleMedia);
          media.push({ ...original, ...mediaUpdate });
          replaced = true;
        } else if (
          singleMedia.type === 'video' &&
          mediaUpdate.type === 'encrypted_video'
        ) {
          const original = _omit([
            'uri',
            'thumbnailURI',
            'localMediaSelection',
          ])(singleMedia);
