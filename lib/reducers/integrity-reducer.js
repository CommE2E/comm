...
function reduceIntegrityStore(
  ...
  if (
    ...
    const threadHashesArray = Object.entries(state.threadHashes).filter(
      ([key]) => {
        // We should not clear the whole store, only remove the threads
        // related to the keyserver that issues state sync
        return (
          extractKeyserverIDFromIDOptional(key) !== action.payload.keyserverID
        );
      },
    );
