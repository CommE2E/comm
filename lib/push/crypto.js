  ...
  try {
    ...
    if (!unencryptedSerializedPayload) {
      return {
        ...
        payloadSizeExceeded: isPayloadSizeValid
          ? isPayloadSizeValid(unencryptedPayload)
          : false,
      };
    }
    ...
    return {
      resultPayload,
      payloadSizeExceeded: isPayloadSizeValid
        ? isPayloadSizeValid(resultPayload)
        : false,
    };
  }
}
...
async function encryptAPNsVisualNotification(
  ...
  try {
    ...
    return {
      ...
      payloadSizeExceeded: isNotificationSizeValid
...
async function encryptAndroidVisualNotification(
  ...
  isNotificationSizeValid?: AndroidVisualNotification => boolean,
  blobHolder?: ?string,
): Promise<{
  +notification: AndroidVisualNotification,
  +payloadSizeExceeded: boolean,
  ...
  if (isNotificationSizeValid) {
    isPayloadSizeValid = (
      ...
      return isNotificationSizeValid({
