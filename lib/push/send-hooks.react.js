...
import { createHTTPAuthorizationHeader } from '../utils/services-utils.js';
...
async function uploadBlobs(
  ...
  const authString = await createHTTPAuthorizationHeader(authMetadata);
  const authHeaders = { Authorization: authString };
  ...
  const assignmentPromise = assignMultipleHolders(assignments, {
    ...authHeaders,
