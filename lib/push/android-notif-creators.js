...
async function createAndroidVisualNotification(
  ...
  const copyWithMessageInfosBlob = JSON.stringify(copyWithMessageInfos);
  const priority = 'high';
  if (!shouldBeEncrypted) {
    const notificationToSend =
      encryptedNotifUtilsAPI.getNotifByteSize(copyWithMessageInfosBlob) <=
  ...
  if (canQueryBlobService && lazyGenerateBlobEncryptionKey) {
    const { encryptionKey: key, initializationVector } =
      await lazyGenerateBlobEncryptionKey();

    const largeNotifData = await prepareLargeNotifData(
  ...
  if (!encryptedCopyWithMessageInfos || !blobHash || !blobHolders) {
