...
> = {
  ...
  +notifCreatorCallback: (
    ...
    lazyGenerateBlobEncryptionKey?: () => Promise<{
...
async function buildNotifsFromPushInfo(
  ...
  let lazyGenerateBlobEncryptionKeyPromise = null;
  const lazyGenerateBlobEncryptionKey = () => {
    if (!lazyGenerateBlobEncryptionKeyPromise) {
      lazyGenerateBlobEncryptionKeyPromise = (async () => {
        const [encryptionKey, initializationVector] = await Promise.all([
          encryptedNotifUtilsAPI.generateAESKey(),
          encryptedNotifUtilsAPI.generateAESIV(),
        ]);
        return { encryptionKey, initializationVector };
      })();
    }
    return lazyGenerateBlobEncryptionKeyPromise;
  };
