...
function createChatMessageItems(
  ...
  if (
    threadInfo &&
    threadSpecs[threadInfo.type].protocol().pinsStoredOnServer
  ) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      (messageInfo.type === messageTypes.UNSUPPORTED &&
        messageInfo.unsupportedMessageInfo.type === messageTypes.PLAIN)
    ) {
      continue;
    }
