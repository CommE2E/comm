...
import {
  ...
  sortMessageInfoList,
  parseSourceMessageIDFromPendingThreadID,
...
function createChatMessageItems(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (isComposableMessageType(originalMessageInfo.type)) {
      ...
      const parentMessageID = parseSourceMessageIDFromPendingThreadID(threadID);
      const parentThreadID = parentMessageID
        ? messageStore.messages[parentMessageID]?.threadID
        : '';
