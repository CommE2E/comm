...
function createChatThreadItem(
  ...
  const orderedSidebars = _orderBy('lastUpdatedTime')('desc')(
    unorderedSidebars,
  );

  const numUnreadSidebars = orderedSidebars.filter(
    (sidebar) => sidebar.threadInfo.currentUser.unread,
  ).length;
  let numReadSidebarsToShow = maxReadSidebars - numUnreadSidebars;

  const sidebarItems = [];
  for (const sidebar of orderedSidebars) {
  ...
  if (sidebarItems.length < orderedSidebars.length) {
    sidebarItems.push({
      type: 'seeMore',
