...
import { threadTypeIsPrivate } from '../shared/threads/thread-specs.js';
import { ensNameForFarcasterUsername } from '../shared/user-utils.js';
...
// expensive. useResolvableNames would trigger their recalculation for each
// ENS name as it streams in, but we would prefer to trigger their
// recaculation just once for every update of the underlying Redux data.
const useResolvableNamesOptions = { allAtOnce: true };

type SearchIndexUserInfo = {
  +id: string,
  +username: ?string,
  +fid?: ?string,
...
function useUserSearchIndex(
  ...
  const membersWithENSNames = useResolvableNames<SearchIndexUserInfo>(
    userInfos,
    useResolvableNamesOptions,
  );

  const memberMap = React.useMemo(() => {
    const result = new Map<string, SearchIndexUserInfo>();
  ...
  return React.useMemo(() => {
    ...
    for (const userInfo of userInfos) {
      ...
      const resolvedFarcasterName = resolvedUserInfo?.farcasterUsername;
      if (
        resolvedFarcasterName &&
        resolvedFarcasterName !== rawUsername &&
        resolvedFarcasterName !== resolvedUsername
      ) {
        searchTextArray.push(
          ensNameForFarcasterUsername(resolvedFarcasterName),
        );
      }
...
function useThreadSearchIndex(
  ...
  const nonViewerMembersWithENSNames = useResolvableNames(
    nonViewerMembers,
    useResolvableNamesOptions,
  );
