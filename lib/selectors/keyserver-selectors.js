...
const baseUrlPrefixSelector: (id: string) => (state: AppState) => ?string =
  (id: string) => (state: AppState) =>
    state.keyserverStore.keyserverInfos[id]?.urlPrefix;

const urlPrefixSelector: (id: string) => (state: AppState) => ?string =
  _memoize(baseUrlPrefixSelector);

const baseConnectionSelector: (
  id: string,
) => (state: AppState) => ?ConnectionInfo = (id: string) => (state: AppState) =>
  state.keyserverStore.keyserverInfos[id]?.connection;

const connectionSelector: (id: string) => (state: AppState) => ?ConnectionInfo =
  _memoize(baseConnectionSelector);

const baseLastCommunicatedPlatformDetailsSelector: (
  id: string,
) => (state: AppState) => ?PlatformDetails =
  (id: string) => (state: AppState) =>
    state.keyserverStore.keyserverInfos[id]?.lastCommunicatedPlatformDetails;
