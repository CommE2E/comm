...
const chatMentionCandidatesSelector: (state: BaseAppState<>) => {
  +[id: string]: $ReadOnlyArray<ThreadInfo>,
} = createSelector(
  threadInfoSelector,
  (threadInfos: { +[id: string]: ThreadInfo }) => {
      ...
      if (threadInfos[currentThreadCommunity].type === threadTypes.GENESIS) {
        ...
        while (currentlySelectedThreadID) {
          ...
          currentlySelectedThreadID =
            currentlySelectedThreadInfo.parentThreadID;
        }
        ...
        if (
          threadInfos[lastThreadParentThreadID].type === threadTypes.GENESIS
        ) {
