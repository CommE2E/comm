...
const baseThreadMembersSelectorForThread: (
  threadID: string,
) => (
  BaseAppState<>,
  ...
  createSelector(
    ...
    (threadInfos: {
      ...
      const selectedThreadInfo = threadInfos[threadID];
      const { community } = selectedThreadInfo;

      const communityThreadInfo = community ? threadInfos[community] : null;
      const topMostThreadInfo =
        communityThreadInfo && communityThreadInfo.type !== threadTypes.GENESIS
          ? communityThreadInfo
          : selectedThreadInfo;

      const { members, roles } = topMostThreadInfo;

      const roleMap = new Map<string, $ReadOnlyArray<RelativeMemberInfo>>();
      ...
      for (const member of members) {
        ...
        const memberList = roleMap.get(memberRole);
        if (!memberList) {
          continue;
        }

        const newMemberList = [...memberList, member];
        roleMap.set(memberRole, newMemberList);
      }
...
const threadMembersSelectorForThread: (
  threadID: string,
) => (
  state: BaseAppState<>,
) => $ReadOnlyMap<string, $ReadOnlyArray<RelativeMemberInfo>> = _memoize(
  baseThreadMembersSelectorForThread,
);
