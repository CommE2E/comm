...
import type {
  ProtocolName,
  ThreadProtocol,
} from '../shared/threads/thread-spec.js';
import {
  threadSpecs,
  personalThreadTypes,
  privateThreadTypes,
...
const createPendingToRealizedThreadIDsSelector = (
  selectedProtocol: ?ThreadProtocol,
) =>
  createSelector(
    ...
    (rawThreadInfos: RawThreadInfos) => {
      ...
      for (const threadID in rawThreadInfos) {
        ...
        if (
          selectedProtocol &&
          threadSpecs[rawThreadInfo.type].protocol()?.protocolName !==
            selectedProtocol
        ) {
          continue;
        }
        ...
        //TODO przeiterowac po protokolach
...
const createPendingToRealizedThreadIDsSelectorMemoized = _memoize(
  createPendingToRealizedThreadIDsSelector,
);
