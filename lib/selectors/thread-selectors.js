...
import type { ThreadProtocol } from '../shared/threads/thread-spec.js';
import {
  threadSpecs,
  personalThreadTypes,
  privateThreadTypes,
...
const unreadFarcasterThreadIDsSelector: (
  state: BaseAppState<>,
) => $ReadOnlyArray<string> = createSelector(
  (state: BaseAppState<>) => state.threadStore.threadInfos,
  (threadInfos: RawThreadInfos): $ReadOnlyArray<string> =>
...
const pendingToRealizedThreadIDsSelector: (
  ...
  (rawThreadInfos: RawThreadInfos) => {
    ...
    for (const threadID in rawThreadInfos) {
      ...
      const possibleProtocols: Array<?ThreadProtocol<any>> = [
        threadSpecs[rawThreadInfo.type].protocol(),
      ];
      if (!rawThreadInfo.farcaster) {
        possibleProtocols.push(null);
      }
