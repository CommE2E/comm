...
import { threadTypeIsPersonal } from '../shared/threads/thread-specs.js';
...
const userInfoSelectorForPotentialMembers: (state: BaseAppState<>) => {
  ...
  (userInfos: UserInfos): { [id: string]: AccountUserInfo } => {
    ...
    for (const id in userInfos) {
      const { username, relationshipStatus, avatar } = userInfos[id];
      if (!username) {
        continue;
      }
      if (
        ...
        availableUsers[id] = { id, username, relationshipStatus, avatar };
      }
    }
...
const getPeersDeadDeviceIDs: (state: BaseAppState<>) => Set<string> =
  createSelector(
    (state: BaseAppState<>) => state.auxUserStore.auxUserInfos,
    (auxUserInfos: AuxUserInfos) =>
      new Set(
        values(auxUserInfos).flatMap(auxUserInfo => {
          const deviceList = auxUserInfo.deviceList?.devices ?? [];
          return deviceList.filter(
            deviceID => !auxUserInfo.devicesPlatformDetails?.[deviceID],
          );
        }),
      ),
  );
