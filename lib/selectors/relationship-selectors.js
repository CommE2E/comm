  ...
  createSelector(
    ...
    (userInfos: UserInfos) => {
      ...
      for (const userID in userInfos) {
        ...
        const { id, username, avatar, relationshipStatus } = userInfo;
        if (!username) {
          continue;
        }
        if (
