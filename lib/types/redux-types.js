// @flow

import type { CalendarInfo } from './calendar-types';
import type { EntryInfo } from './entry-types';
import type { LoadingStatus, LoadingInfo } from './loading-types';
import type { BaseNavInfo } from './nav-types';
import type { UserInfo } from './user-types';
import type { LogInResult } from '../actions/user-actions';
import type { PingResult } from '../actions/ping-actions';
import type { EntriesResult } from '../actions/entry-actions';

export type BaseAppState = {
  // This "+" means that navInfo can be a sub-type of BaseNavInfo. As a result,
  // within lib (where we're handling a generic BaseAppState) we can read
  // navInfo, but we can't set it - otherwise, we may be setting a more specific
  // type to a more general one.
  +navInfo: BaseNavInfo,
  userInfo: ?UserInfo,
  entryInfos: {[id: string]: EntryInfo},
  daysToEntries: {[day: string]: string[]},
  // This indicates when the entryInfos within the range specified by startDate
  // and endDate (in navInfo) were last updated. An update on any of the
  // contained entryInfos will reset this field to the current timestamp in ms.
  // We only use this on native, where the range between startDate and endDate
  // can grow large and we want to periodically reset it instead of trying to
  // maintain a current value for all the entryInfos within the range.
  entriesWithinRangeLastUpdated: number,
  calendarInfos: {[id: string]: CalendarInfo},
  loadingStatuses: {[key: string]: {[idx: number]: LoadingStatus}},
  cookie: ?string,
};

export type BaseAction =
  { type: "@@redux/INIT" } | {
    type: "FETCH_ENTRIES_AND_SET_RANGE_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_ENTRIES_AND_SET_RANGE_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_ENTRIES_AND_SET_RANGE_SUCCESS",
    payload: EntriesResult,
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_ALL_DAY_ENTRIES_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_ALL_DAY_ENTRIES_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_ALL_DAY_ENTRIES_SUCCESS",
    payload: EntryInfo[],
    loadingInfo: LoadingInfo,
  } | {
    type: "LOG_OUT_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "LOG_OUT_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "LOG_OUT_SUCCESS",
    payload: {[id: string]: CalendarInfo},
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_ACCOUNT_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_ACCOUNT_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_ACCOUNT_SUCCESS",
    payload: {[id: string]: CalendarInfo},
    loadingInfo: LoadingInfo,
  } | {
    type: "CREATE_LOCAL_ENTRY",
    payload: EntryInfo,
  } | {
    type: "SAVE_ENTRY_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "SAVE_ENTRY_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "SAVE_ENTRY_SUCCESS",
    payload: {
      localID: ?string,
      serverID: string,
      text: string,
    },
    loadingInfo: LoadingInfo,
  } | {
    type: "CONCURRENT_MODIFICATION_RESET",
    payload: {
      id: string,
      dbText: string,
    },
  } | {
    type: "DELETE_ENTRY_STARTED",
    loadingInfo: LoadingInfo,
    payload: {
      localID: ?string,
      serverID: ?string,
    },
  } | {
    type: "DELETE_ENTRY_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_ENTRY_SUCCESS",
    loadingInfo: LoadingInfo,
  } | {
    type: "LOG_IN_STARTED",
    loadingInfo: LoadingInfo,
    payload: {
      source?: "COOKIE_INVALIDATION_RESOLUTION_ATTEMPT" |
        "APP_START_NATIVE_CREDENTIALS_AUTO_LOG_IN" |
        "APP_START_REDUX_LOGGED_IN_BUT_INVALID_COOKIE",
    },
  } | {
    type: "LOG_IN_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "LOG_IN_SUCCESS",
    payload: LogInResult,
    loadingInfo: LoadingInfo,
  } | {
    type: "REGISTER_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "REGISTER_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "REGISTER_SUCCESS",
    payload: UserInfo,
    loadingInfo: LoadingInfo,
  } | {
    type: "RESET_PASSWORD_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "RESET_PASSWORD_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "RESET_PASSWORD_SUCCESS",
    payload: LogInResult,
    loadingInfo: LoadingInfo,
  } | {
    type: "FORGOT_PASSWORD_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "FORGOT_PASSWORD_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "FORGOT_PASSWORD_SUCCESS",
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_USER_SETTINGS_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_USER_SETTINGS_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_USER_SETTINGS_SUCCESS",
    payload: {
      email: string,
    },
    loadingInfo: LoadingInfo,
  } | {
    type: "RESEND_VERIFICATION_EMAIL_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "RESEND_VERIFICATION_EMAIL_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "RESEND_VERIFICATION_EMAIL_SUCCESS",
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_CALENDAR_SETTINGS_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_CALENDAR_SETTINGS_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "CHANGE_CALENDAR_SETTINGS_SUCCESS",
    payload: CalendarInfo,
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_CALENDAR_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_CALENDAR_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "DELETE_CALENDAR_SUCCESS",
    payload: string,
    loadingInfo: LoadingInfo,
  } | {
    type: "NEW_CALENDAR_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "NEW_CALENDAR_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "NEW_CALENDAR_SUCCESS",
    payload: CalendarInfo,
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_REVISIONS_FOR_ENTRY_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_REVISIONS_FOR_ENTRY_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "FETCH_REVISIONS_FOR_ENTRY_SUCCESS",
    payload: {
      entryID: string,
      text: string,
      deleted: bool,
    },
    loadingInfo: LoadingInfo,
  } | {
    type: "RESTORE_ENTRY_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "RESTORE_ENTRY_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "RESTORE_ENTRY_SUCCESS",
    payload: string, // entryID
    loadingInfo: LoadingInfo,
  } | {
    type: "AUTH_CALENDAR_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "AUTH_CALENDAR_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "AUTH_CALENDAR_SUCCESS",
    payload: CalendarInfo,
    loadingInfo: LoadingInfo,
  } | {
    type: "SUBSCRIBED_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "SUBSCRIBE_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "SUBSCRIBE_SUCCESS",
    payload: {
      calendarID: string,
      newSubscribed: bool,
    },
    loadingInfo: LoadingInfo,
  } | {
    type: "SET_COOKIE",
    payload: {
      cookie: ?string,
      // We make sure the server includes calendarInfos in any response that
      // triggers this action. This is here (instead of in the _SUCCESS action
      // corresponding to the async call) because any endpoint can trigger a
      // cookie invalidation. However, most SET_COOKIE actions aren't cookie
      // invalidations; more commonly, SET_COOKIE gets dispatched as a
      // consequence of other actions, such as a log in. In those cases, the
      // _SUCCESS action (eg. LOG_IN_SUCCESS) will also have the CalendarInfos
      // (and a UserInfo) in its payload, but note that SET_COOKIE will get
      // triggered first. The reducer for the _SUCCESS action should thus do a
      // deep (recursive) equality check before setting CalendarInfos. UserInfo
      // isn't included in the SET_COOKIE payload because we know cookie
      // invalidations correspond to null userInfos, and in all other cases of
      // cookies being changed, the _SUCCESS action will handle the UserInfo.
      calendarInfos?: {[id: string]: CalendarInfo},
      // Whether a user triggered this SET_COOKIE action or it's the result of
      // a cookie invalidation
      cookieInvalidated?: bool,
    },
  } | {
    type: "PING_STARTED",
    loadingInfo: LoadingInfo,
  } | {
    type: "PING_FAILED",
    error: true,
    payload: Error,
    loadingInfo: LoadingInfo,
  } | {
    type: "PING_SUCCESS",
    payload: PingResult,
    loadingInfo: LoadingInfo,
  };

type ThunkedAction<T: BaseAppState, A> = (dispatch: Dispatch<T, A>) => void;
type PromisedAction<T: BaseAppState, A> =
  (dispatch: Dispatch<T, A>) => Promise<void>;
export type Dispatch<T: BaseAppState, A> =
  ((promisedAction: PromisedAction<T, BaseAction>) => Promise<void>) &
  ((promisedAction: PromisedAction<T, A>) => Promise<void>) &
  ((thunkedAction: ThunkedAction<T, BaseAction>) => void) &
  ((thunkedAction: ThunkedAction<T, A>) => void) &
  ((action: BaseAction) => BaseAction) &
  ((action: A) => A);
