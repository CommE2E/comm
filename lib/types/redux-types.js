...
export type BaseAction =
  ...
  |} | {|
    ...
    payload: {|
      ...
      // (and a CurrentUserInfo) in its payload, but note that SET_COOKIE will
      // get triggered first. The reducer for the _SUCCESS action should thus do
      // a deep (recursive) equality check before setting ThreadInfos.
      // CurrentUserInfo isn't included in the SET_COOKIE payload because we
      // know cookie invalidations correspond to null userInfos, and in all
      // other cases of cookies being changed, the _SUCCESS action will handle
      // the CurrentUserInfo.
      threadInfos?: {[id: string]: ThreadInfo},
