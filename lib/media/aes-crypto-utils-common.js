...
async function generateIVCommon(commonCrypto: Crypto): Promise<Uint8Array> {
  return commonCrypto.getRandomValues(new Uint8Array(IV_LENGTH));
}
...
async function encryptCommon(
  ...
  if (initializationVector && initializationVector.length !== IV_LENGTH) {
  ...
  const iv = initializationVector
    ...
    : commonCrypto.getRandomValues(new Uint8Array(outputBuffer, 0, IV_LENGTH));
...
export { generateKeyCommon, encryptCommon, decryptCommon, generateIVCommon };
