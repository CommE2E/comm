...
function FarcasterDataHandler(props: Props): React.Node {
  ...
  const [shouldSync, setShouldSync] = React.useState(true);
  ...
  React.useEffect(() => {
    ...
    setShouldSync(false);
    void syncFarcasterConversations();
  }, [
    currentUserSupportsDCs,
    shouldSync,
  ...
  React.useEffect(() => {
    if (!currentUserSupportsDCs && !shouldSync) {
      setShouldSync(true);
    }
  }, [currentUserSupportsDCs, shouldSync]);
