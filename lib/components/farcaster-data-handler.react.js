...
function FarcasterDataHandler(props: Props): React.Node {
    ...
    async (userIDs: $ReadOnlyArray<string>) => {
      for (const userID of userIDs) {
        await dispatchActionPromise(
          ...
          updateRelationships(relationshipActions.FARCASTER_MUTUAL, [userID]),
        );
      }
    },
    [dispatchActionPromise, updateRelationships],
  );
  ...
  const handleFarcasterMutualsIsRunningRef = React.useRef(false);
  const handleFarcasterMutuals = React.useCallback(async () => {
    ...
    if (
      ...
      !getFarcasterUsers ||
      !currentUserID ||
      handleFarcasterMutualsIsRunningRef.current
    ) {
      return;
    }

    handleFarcasterMutualsIsRunningRef.current = true;
    ...
    try {
      ...
      await createThreadsAndRobotextForFarcasterMutuals(newCommUserIDs);
    } finally {
      handleFarcasterMutualsIsRunningRef.current = false;
    }
  }, [
    isActive,
    fid,
