...
import { setSessionRecoveryInProgressActionType } from '../keyserver-conn/keyserver-conn-types.js';
...
function KeyserverConnectionHandler(props: Props) {
  ...
  const urlPrefix = useSelector(
    state => state.keyserverStore.keyserverInfos[keyserverID]?.urlPrefix,
  );
  const performRecovery = React.useCallback(() => {
    ...
    const promise = (async () => {
      try {
        ...
        console.log(
          `Error while recovery session with keyserver id ${keyserverID}`,
          e,
        );
