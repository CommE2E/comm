...
function FarcasterChannelPrefetchHandler(): React.Node {
  ...
  React.useEffect(() => {
    ...
    for (const threadID in rawThreadInfos) {
      ...
      if (!communityInfos[communityID]) {
        console.log(
          `A community ${communityID} that contains a thread ${threadID}` +
            " doesn't exist in the communityStore",
        );
        continue;
      }
      ...
      if (farcasterChannelID && !channelIDs.has(farcasterChannelID)) {
        ...
        channelIDs.add(farcasterChannelID);
      }
    }
