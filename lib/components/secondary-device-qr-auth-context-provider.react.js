...
function SecondaryDeviceQRAuthContextProvider(props: Props): React.Node {
  ...
  if (prevLoggedIn.current !== loggedIn) {
    console.log('[context] Logged in state changed: loggedIn =', loggedIn);
    if (!loggedIn) {
      logoutStateResetPromise.current = (async () => {
        console.log('[context promise] Inside promise');
        await logoutStateResetPromise.current;
        console.log('[context promise] Waiting for DB');
        await waitUntilDatabaseDeleted();
        console.log('[context promise] DB deleted');
  ...
  const openSecondaryQRAuth = React.useCallback(async () => {
    console.log('[QR gen] Waiting until DB ready');
    await logoutStateResetPromise.current;

    console.log('[QR gen] Getting signing key');
    ...
    console.log('[QR gen] Set deviceID', ed25519);
