...
import { convertStoreOperationsToClientDBStoreOperations } from '../shared/redux/client-db-utils.js';
  ...
  const restoreUserData = React.useCallback(
    async (
      ...
      try {
        ...
        function migrateThreadsColorAndDescription(
          store: ClientStore,
        ): StoreOperations {
          console.log(store);
          const threadInfos: Array<RawThreadInfo> = Object.values(
            store.threadStore.threadInfos,
          );

          console.log(threadInfos);
          ...
          console.log(threadStoreOperations);

          return { threadStoreOperations };
        }

        //#### 1. Restore
        console.log('restoring and applying logs');
        await sqliteAPI.restoreUserData(backupData, identityAuthResult);

        //#### 2. Migrate schema
        const clientDBStore1 = await sqliteAPI.getClientDBStore(
          databaseIdentifier.RESTORED,
          identityAuthResult.userID,
        );
        console.log(
          'synced metadata before migrating schema',
          clientDBStore1.syncedMetadata,
        );
        await sqliteAPI.migrateSchema();
        const clientDBStore2 = await sqliteAPI.getClientDBStore(
          databaseIdentifier.RESTORED,
          identityAuthResult.userID,
        );
        console.log(
          'synced metadata before migrating schema',
          clientDBStore2.syncedMetadata,
        );
        ...
        const ops: StoreOperations =
          migrateThreadsColorAndDescription(clientStoreToMigrate);
        ...
        //#### 4. Copy content
        console.log(' //#### 4. Copy content');
        await sqliteAPI.copyContentFromBackupDatabase();

        //#### 5. Populate store
        console.log('//#### 5. Populate store');
