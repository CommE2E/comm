...
export type MediaCachePersistence = {
  ...
  +getCachedURI: (holder: string) => Promise<?string>,
...
function createMediaCacheContext(
  ...
  async function get(holder: string): Promise<?string> {
    let cachedFile = uriCache.get(holder);
    if (cachedFile) {
      ...
      if (await persistence.hasURI(cachedFile)) {
        return cachedFile;
      } else {
    ...
    cachedFile = await persistence.getCachedURI(holder);
    if (cachedFile) {
