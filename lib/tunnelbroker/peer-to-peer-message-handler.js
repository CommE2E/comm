...
function PeerToPeerMessageHandler(props: Props): React.Node {
  ...
  const processingInputMessagesStatus = React.useRef<
  ...
  React.useEffect(() => {
    void (async () => {
      ...
      if (processingInputMessagesStatus.current === 'pending') {
        processingInputMessagesStatus.current = 'in_progress';
        await processPersistedInboundMessages();
        processingInputMessagesStatus.current = 'done';
      }
      if (processingInputMessagesStatus.current === 'done') {
