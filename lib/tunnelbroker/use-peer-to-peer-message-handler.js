...
import { useConfirmPeerToPeerMessage } from './use-confirm-peer-to-peer-message.js';
import {
  useResendPeerToPeerMessages,
  useResendP2PMessagesToNewPrimaryDevices,
} from './use-resend-peer-to-peer-messages.js';
...
function useHandleOlmMessageToDevice(): (
  ...
  const removeAndConfirmMessage = React.useCallback(
    async (messageID: string, senderDeviceID: string) => {
      const { sqliteAPI } = getConfig();

      await Promise.all([
        sqliteAPI.removeInboundP2PMessages([messageID]),
  ...
  return React.useCallback(
    async (
      ...
      if (
        ...
        userActionMessage.type === userActionsP2PMessageTypes.ADD_KEYSERVER
      ) {
        const { keyserverAdminUserID, urlPrefix } = userActionMessage;
        dispatch({
          type: addKeyserverActionType,
          payload: {
            keyserverAdminUserID,
            newKeyserverInfo: defaultKeyserverInfo(urlPrefix),
          },
        });
      } else if (
        userActionMessage.type === userActionsP2PMessageTypes.REMOVE_KEYSERVER
      ) {
        const { keyserverAdminUserID } = userActionMessage;
        dispatch({
...
function usePeerToPeerMessageHandler(): (
  ...
  const handlePrimaryDeviceChanges = useResendP2PMessagesToNewPrimaryDevices();
  const { createOlmSessionsWithUser } = usePeerOlmSessionsCreatorContext();

  const addNewFarcasterMessage = useAddNewFarcasterMessage();
  const handleFarcasterInboxStatus = useHandleFarcasterInboxStatus();
  ...
  return React.useCallback(
    async (message: PeerToPeerMessage, messageID: string) => {
      if (message.type === peerToPeerMessageTypes.OUTBOUND_SESSION_CREATION) {
        ...
        try {
          ...
          if (
            1 ||
            errorMessage.includes(olmSessionErrors.invalidSessionVersion)
          ) {
            resultDescription =
        ...
        message.type === peerToPeerMessageTypes.FARCASTER_INBOX_STATUS
      ) {
        try {
          await handleFarcasterInboxStatus(
            message.unreadConversationIds,
            message.readConversationIds,
          );
        } catch (e) {
          console.log(
            `Error processing Farcaster inbox status: ${
              getMessageForException(e) ?? 'unknown error'
            }`,
          );
        }
    ...
    [
      ...
      handlePrimaryDeviceChanges,
      sqliteAPI,
      addNewFarcasterMessage,
      handleFarcasterInboxStatus,
    ],
  );
}
