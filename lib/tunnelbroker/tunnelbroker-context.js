...
type PromiseRequestCallbacks = {
  ...
  +reject: (
    error: string | FarcasterAPIError | FarcasterMissingTokenError,
  ) => void,
};
...
const defaultSocketState: TunnelbrokerSocketState = {
  connected: false,
  retryCount: 0,
};
...
type TunnelbrokerContextType = {
  ...
  +sendFarcasterRequest: (request: FarcasterAPIRequest) => Promise<string>,
};
...
function TunnelbrokerProvider(props: Props): React.Node {
  ...
  const [socketState, setSocketState] =
    React.useState<TunnelbrokerSocketState>(defaultSocketState);
  const socketStateRef =
    React.useRef<TunnelbrokerSocketState>(defaultSocketState);

  const resetSocketState = React.useCallback(() => {
    socketStateRef.current = defaultSocketState;
    setSocketState(defaultSocketState);
  }, []);
  ...
  }, [resetSocketState, stopHeartbeatTimeout]);
      ...
      try {
        ...
        tunnelbrokerSocket.onclose = () => {
          ...
          setSocketState(prev => {
            ...
            socketStateRef.current = newValue;
            return newValue;
          });
          onClose?.();
          socket.current = null;
        ...
        tunnelbrokerSocket.onmessage = (event: MessageEvent) => {
          ...
          if (
            ...
            if (message.status.type === 'Success' && !socketState.connected) {
              socketStateRef.current = { connected: true, isAuthorized };
              ...
              if (message.status.data?.includes('UnauthorizedDevice')) {
                void invalidTokenLogOut('tunnelbroker_unauthorized_device');
                return;
              }
              resetSocketState();
              console.log(
            ...
            if (response.type === 'Success') {
              ...
              requests.current[requestID].reject(response.data);
              delete requests.current[requestID];
  ...
  const sendMessage: (
    identifier: string,
    request: DeviceToTunnelbrokerRequest,
  ) => Promise<void> = React.useCallback(
    (identifier: string, request: DeviceToTunnelbrokerRequest) => {
      const resultPromise: Promise<void> = new Promise((resolve, reject) => {
        ...
        if (socketActive) {
          ...
          secondaryTunnelbrokerConnection?.sendMessage(identifier, request);
        }
      });
    ...
    [secondaryTunnelbrokerConnection, shouldBeClosed],
  );
  ...
  const sendMessageToDevice: (
    ...
    (message: TunnelbrokerClientMessageToDevice, messageID: ?string) => {
      ...
      return sendMessage(clientMessageID, messageToDevice);
    },
    [sendMessage],
  );
  ...
  const sendMessageToTunnelbroker: (payload: string) => Promise<void> =
    React.useCallback(
      (payload: string) => {
        ...
        return sendMessage(clientMessageID, messageToTunnelbroker);
      },
      [sendMessage],
    );

  const sendNotif: (notif: TunnelbrokerNotif) => Promise<void> =
    React.useCallback(
      (notif: TunnelbrokerNotif) => {
        return sendMessage(notif.clientMessageID, notif);
      },
      [sendMessage],
    );
  ...
  const sendFarcasterRequest: (
    ...
    request => {
      const resultPromise: Promise<string> = new Promise((resolve, reject) => {
        ...
        requests.current[request.requestID] = {
          resolve,
          reject,
        };
        if (socketActive) {
          ...
          secondaryTunnelbrokerConnection?.sendMessage(
            request.requestID,
            request,
          );
        }
  ...
  React.useEffect(
    () =>
      secondaryTunnelbrokerConnection?.onSendMessage(
        (identifier: string, message: DeviceToTunnelbrokerRequest) => {
          if (shouldBeClosed) {
          ...
          void (async () => {
            try {
              ...
              secondaryTunnelbrokerConnection.setMessageStatus(identifier);
            } catch (error) {
  ...
  const confirmMessageToTunnelbroker = React.useCallback(
    (messageID: string) => {
      ...
      addLog(
        'Tunnelbroker',
        `Confirmed a message with ID: ${messageID}`,
        new Set([logTypes.TUNNELBROKER]),
      );
