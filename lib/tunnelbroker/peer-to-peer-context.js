...
type QueueItem =
  | {
      +type: 'process_outbound_messages',
      +message: QueuedMessage,
    }
  | {
      +type: 'send_notifs',
      +message: QueuedMessage,
      +processingResult: ProcessOutboundP2PMessagesResult,
    }
  | {
      +type: 'resolve',
      +message: QueuedMessage,
      +result: ProcessOutboundP2PMessagesResult,
    };
...
function PeerToPeerProvider(props: Props): React.Node {
  ...
  const processItem = React.useCallback(
    async (item: QueueItem, enqueue: ($ReadOnlyArray<QueueItem>) => mixed) => {
      const { dmOpID } = item.message;
      try {
        if (item.type === 'process_outbound_messages') {
          const result = await processOutboundP2PMessages(
            sendMessageToDevice,
            identityContext,
            peerOlmSessionsCreator,
            item.message.outboundMessageIDs,
            item.message.allPeerDevices,
          );
          enqueue([
            {
              type: 'send_notifs',
        ...
        } else if (item.type === 'send_notifs') {
          await sendPushNotifs(item.message.notificationsCreationData);
          enqueue([
            {
              type: 'resolve',
              message: item.message,
              result: item.processingResult,
            },
          ]);
        } else {
          if (dmOpID) {
