...
import {
  ...
  getNotificationsCreationDataFromDMOp,
  ...
  const sendDMOperation = React.useCallback(
    async (op: DMOperationSpecification) => {
      ...
      const notificationsCreationData =
        await getNotificationsCreationDataFromDMOp(op, utilities);
    ...
    [utilities, allPeerUserIDAndDeviceIDs, currentUserInfo, dispatch],
  );
