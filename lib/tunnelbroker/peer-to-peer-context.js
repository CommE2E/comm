...
function PeerToPeerProvider(props: Props): React.Node {
  ...
  const processOutboundMessages = React.useCallback(
    (outboundMessageIDs: ?$ReadOnlyArray<string>, dmID: ?string) => {
      processingQueue.current.push({ outboundMessageIDs, dmID });
        ...
        void (async () => {
          do {
            ...
            try {
              ...
              if (queueFront.dmID) {
                dmOpsSendingPromiseResolvers.current.delete(queueFront.dmID);
              }
            }
