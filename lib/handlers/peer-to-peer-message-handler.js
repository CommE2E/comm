...
import { syncingP2PMessageValidator } from '../types/tunnelbroker/syncing-peer-to-peer-message-types.js';
import type { SyncingP2PMessage } from '../types/tunnelbroker/syncing-peer-to-peer-message-types.js';
  ...
  if (message.type === peerToPeerMessageTypes.OUTBOUND_SESSION_CREATION) {
    ...
    try {
      ...
      try {
        ...
        if (!syncingP2PMessageValidator.is(parsedMessageToDevice)) {
          return;
        }
        const syncingP2PMessage: SyncingP2PMessage = parsedMessageToDevice;
        const action = getActionFromOutboundP2PMessage(syncingP2PMessage);
        if (action) {
          dispatch(action, {
            messageID,
            senderDeviceID: message.senderInfo.deviceID,
          });
        }
      } catch (e) {
        console.log(e);
