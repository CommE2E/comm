    ...
    (inputs: KeyserverAuthInputs) =>
      async (innerCallKeyserverEndpoint: CallKeyserverEndpoint) => {
        ...
        try {
          ...
          if (getMessageForException(e) === 'user_not_found') {
            // We want to distinguish user_not_found returned by
            // the GetKeyserverKeys RPC from regular user_not_found
            // which means invalid username provided by user.
            throw new Error('keyserver_owner_not_found');
          }
          throw e;
        } finally {
          if (!hasBeenCancelled()) {
