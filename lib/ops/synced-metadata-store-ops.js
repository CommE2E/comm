...
export type ClientDBReplaceSyncedMetadataEntryOperation = {
  +type: 'replace_synced_metadata_entry',
  +payload: ClientDBSyncedMetadataEntry,
};

export type ClientDBSyncedMetadataStoreOperation =
  | ClientDBReplaceSyncedMetadataEntryOperation
  | RemoveSyncedMetadataOperation
  | RemoveAllSyncedMetadataOperation;
...
> = {
  ...
  convertOpsToClientDBOps(
    ...
    return ops.map(operation => {
      if (
        operation.type === 'remove_synced_metadata' ||
        operation.type === 'remove_all_synced_metadata'
      ) {
        return operation;
      }
