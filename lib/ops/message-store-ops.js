...
import {
  ...
  translateLocalMessageInfoToClientDBLocalMessageInfo,
...
export type ReplaceMessageStoreLocalOperation = {
  +type: 'replace_local_message_info',
  +payload: { +id: string, +messageInfo: LocalMessageInfo },
};

export type RemoveMessageStoreLocalOperation = {
  +type: 'remove_local_message_infos',
  +payload: { +ids: $ReadOnlyArray<string> },
};

export type RemoveAllMessageStoreLocalOperation = {
...
export type ClientDBReplaceLocalOperation = {
  +type: 'replace_local_message_info',
...
export type MessageStoreOperation =
  ...
  | ReplaceMessageStoreLocalOperation
  | RemoveMessageStoreLocalOperation
  | RemoveAllMessageStoreLocalOperation;
  ...
  processStoreOperations(
    ...
    for (const operation of ops) {
      if (operation.type === 'replace') {
        ...
        processedLocal[operation.payload.id] = operation.payload.messageInfo;
  ...
  convertOpsToClientDBOps(
    ...
    const convertedOperations = ops.map(messageStoreOperation => {
      ...
      if (messageStoreOperation.type === 'replace_threads') {
        ...
        return {
          type: 'replace_local_message_info',
          payload: translateLocalMessageInfoToClientDBLocalMessageInfo(
            messageStoreOperation.payload.id,
            messageStoreOperation.payload.messageInfo,
          ),
        };
      }
