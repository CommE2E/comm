...
//TODO: this can be merged with createFarcasterRawThreadInfo
function createFarcasterRawThreadInfoPersonal(
  ...
  const userIDs = threadInfo.members.map(member => member.id);
  ...
  const avatar: ?ClientAvatar = null;

  //TODO: user proper name and description
  //NOTE: this doesn't really matter, right after creating thread is
  // going to be fetched and updated
  const name = '1:1';
  ...
  return {
    ...
    repliesCount: 0,
    pinnedCount: threadInfo.creationTime,
    minimallyEncoded: true,
    members,
    roles,
    currentUser,
