...
import { backupEnabledByDefault } from './services-utils.js';
import {
...
function createAsyncMigrate<N: BaseNavInfo, T: BaseAppState<N>>(
  ...
  return async function (
    ...
    const { state: newState, schemaChanged } = await runMigrations(
      legacyMigrations,
      migrations,
      state,
      inboundVersion,
    ...
    if (
      ...
      await getConfig().olmAPI.initializeCryptoAccount();
      await getConfig().createBackupCompaction?.(state.currentUserInfo);
    }
