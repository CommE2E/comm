...
class ENSCache {
  ...
  batchReverseResolverSmartContractPromise:
    Promise<ReverseRecordsEthersSmartContract>;
  ...
  getNamesForAddresses(
    ...
    const fetchENSNamesPromise = (async () => {
      ...
      try {
        ...
        invariant(Array.isArray(raceResult), 'test');
        ensNames = raceResult;
      } catch (e) {
        console.log(e);
      ...
      for (let i = 0; i < needFetch.length; i++) {
        ...
        let ensName = ensNames[i]; // what does the contract fill in if there are no ENS names?
