...
import { ServerError, FetchTimeout } from './fetch-utils';
...
export type FetchJSON = (url: string, input: Object) => Promise<Object>;
...
async function fetchJSON(
  ...
  if (possibleReplacement) {
    return await possibleReplacement(url, input);
  }
  ...
  const rejectPromise = (async () => {
    await sleep(10000);
    throw new FetchTimeout(`fetchJSON timed out call to ${url}`, url);
  })();
