...
import type { AddLogCallback } from '../components/debug-logs-context.js';
import { logTypes } from '../components/debug-logs-context.js';
...
import { createMessageReply } from '../shared/markdown.js';
...
function convertFarcasterMessageToCommMessages(
  ...
  if (farcasterMessage.isDeleted) {
    result.push({
      type: messageTypes.DELETE_MESSAGE,
      id: `${farcasterMessage.messageId}/delete`,
      threadID,
      creatorID,
      time: parseInt(farcasterMessage.serverTimestamp, 10) + 2,
  ...
  if (
    ...
    let messageText = '';
    if (farcasterMessage.inReplyTo) {
      messageText += createMessageReply(farcasterMessage.inReplyTo.message);
    }
    messageText += farcasterMessage.message;
    ...
    result.push({
      ...
      id: `${farcasterMessage.messageId}/unsupported`,
      threadID,
      creatorID,
      time: parseInt(farcasterMessage.serverTimestamp, 10),
      robotext: 'Unsupported message type',
      unsupportedMessageInfo: {
        ...
        creatorID,
        time: parseInt(farcasterMessage.serverTimestamp, 10),
        rawContent: JSON.stringify(farcasterMessage),
      },
    });
  }
