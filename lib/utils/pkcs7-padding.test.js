...
import { pad, unpad, testing, calculatePaddedLength } from './pkcs7-padding.js';
...
describe('calculatePaddingLength', () => {
  it('should calculate the correct padding length', () => {
    // we're using a 16 byte block size and 4 block superblocks (64 bytes total)
    // let's say our data is 83 bytes long (1 full superblock + 19 bytes)
    // we need to pad it to be equal to 2 superblocks (2 * 64 = 128 bytes)
    // expected padding length is 128-83 = 45 bytes
    const blockSizeBytes = 16;
    const superblockSizeBlocks = 4;

    const inputLength = 83;
    const expectedPaddedLength = 128;

    const calculatedLength = calculatePaddedLength(inputLength, {
      blockSizeBytes,
      superblockSizeBlocks,
    });

    expect(calculatedLength).toBe(expectedPaddedLength);
  });
});
...
function generateRandomArray(length: number): Array<number> {
  return new Array<void>(length).map(() => Math.floor(Math.random() * 256));
}
