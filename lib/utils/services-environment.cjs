...
function getServicesEnvironmentConfig() {
  try {
    return require('../facts/services-environment.json');
  } catch (e) {
    if (
      (e.code === 'MODULE_NOT_FOUND' &&
        e.message.includes('services-environment.json')) ||
      e.message.includes('Requiring unknown module')
    ) {
      return null;
    }
    throw new Error(`Could not parse services-environment.json: ${e.message}`);
  }
}
...
function resolveServicesEnvironment() {
  ...
  if (
    ...
    throw new Error(`Missing \`environment\` in services-environment.json`);
  }
  return parseServicesEnvironmentOrThrow(
    parsedConfig.environment,
    `services-environment.json (\`environment\`)`,
  );
}
