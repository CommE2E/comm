...
import type { ProtocolIdentifier } from '../shared/threads/thread-spec.js';
import { protocolIdentifierValidator } from '../shared/threads/thread-spec.js';

const protocolIdentifierRegexString = 'dm|dc|keyserver';
...
export const urlInfoValidator: TInterface<URLInfo> = tShape<URLInfo>({
  ...
  // selectedProtocol: t.maybe(protocolIdentifierValidator),
});
...
const threadCreationWithProtocolRegex = new RegExp(
  '(/|^)thread/new/(dm|dc|keyserver)(/([0-9]+([+][0-9]+)*))?(/|$)',
  'i',
);
...
function infoFromURL(url: string): URLInfo {
  ...
  // const threadCreationWithProtocolMatched =
  //   threadCreationWithProtocolRegex.exec(url);
  ...
  // if (threadCreationWithProtocolMatched) {
  //   console.log('threadCreationWithProtocolMatched');
  //   returnObj.threadCreation = true;
  //   const protocol = threadCreationWithProtocolMatched[2];
  //   if (protocol === 'dm' || protocol === 'dc' || protocol === 'keyserver') {
  //     returnObj.selectedProtocol = protocol;
  //   }
  //   returnObj.selectedUserList =
  //     threadCreationWithProtocolMatched[4]?.split('+') ?? [];
  //   console.log(returnObj.selectedUserList, 'selectedUserList');
  // } else
  if (threadCreateMatches) {
    console.log('threadCreateMatches');
    returnObj.threadCreation = true;
...
const setURLPrefix = 'SET_URL_PREFIX' as const;
