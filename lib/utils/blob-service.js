...
function makeFarcasterBlobMediaURI(
  mediaID: string,
  metadata: { +thumbHash?: ?string } = {},
): string {
  const { thumbHash } = metadata;

  const baseURL = makeBlobServiceEndpointURL(
    blobServiceConfig.httpEndpoints.GET_MEDIA,
    { mediaID },
  );
  ...
  // encode base64-url + urlencode padding
  const encodedThumbHash = thumbHash
    .replaceAll('+', '-')
    .replaceAll('/', '_')
    .replaceAll('=', '%3D');

  const urlFragment = `#thumbHash=${encodedThumbHash}`;
