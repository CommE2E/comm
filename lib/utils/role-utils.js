...
function useMembersGroupedByRole(
  threadInfo: ThreadInfo,
): $ReadOnlyArray<GroupedRoleInfo> {
  const { members } = threadInfo;
  const roles = useRolesFromCommunityThreadInfo(threadInfo, members);

  const groupByRoleName = React.useMemo(
    () => _groupBy(member => roles.get(member.id)?.name)(members),
    [members, roles],
  );

  const groupedRoleInfos = _toPairs(groupByRoleName)
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([roleName, memberInfos]) => ({
      roleName,
      memberInfos,
    }));

  return groupedRoleInfos;
}
