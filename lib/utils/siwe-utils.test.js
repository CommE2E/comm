...
import { SIWEMessageTypes } from '../types/siwe-types.js';
...
describe('SIWE Nonce utils', () => {
  ...
  it(`getSIWEStatementForPublicKey should generate expected statements`, () => {
    const validPublicKey = 'rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo';
    const expectedStringForSocialProof =
      `Primary device IdPubKey: rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo ` +
      `By continuing, I accept the Comm Terms of Service: https://comm.app/terms`;
    expect(
      getSIWEStatementForPublicKey(
        validPublicKey,
        SIWEMessageTypes.MSG_AUTH,
      ) === expectedStringForSocialProof,
    ).toBe(true);

    const expectedStringForBackupMessage =
    ...
    expect(
      ...
      ) === expectedStringForBackupMessage,
    ).toBe(true);
  });
  ...
  it(`isValidSIWEStatementWithPublicKey should be true for well formed SIWE statement`, () => {
    ...
    expect(
      isValidSIWEStatementWithPublicKey(expectedStringForSocialProof),
    ).toBe(true);
    expect(
      isValidSIWEStatementWithPublicKey(
        getSIWEStatementForPublicKey(validPublicKey, SIWEMessageTypes.MSG_AUTH),
      ),
    ).toBe(true);
    ...
    expect(
      isValidSIWEStatementWithPublicKey(expectedStringForBackupMessage),
    ).toBe(true);
    expect(
      isValidSIWEStatementWithPublicKey(
        getSIWEStatementForPublicKey(
  ...
  it(`getPublicKeyFromSIWEStatement should pull public key out of valid SIWE statements`, () => {
    const validSIWEMessageStatementForSocialProof =
      `Primary device IdPubKey: rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo ` +
      `By continuing, I accept the Comm Terms of Service: https://comm.app/terms`;

    const validSIWEStatementForBackupMessage =
      `Backup message for primary device IdPubKey: ` +
      `rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo ` +
      `By continuing, I accept the Comm Terms of Service: https://comm.app/terms`;

    expect(
      getPublicKeyFromSIWEStatement(validSIWEMessageStatementForSocialProof),
    ).toBe('rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo');

    expect(
      getPublicKeyFromSIWEStatement(validSIWEStatementForBackupMessage),
    ).toBe('rPFzRtV7E6v1b60zjTvghqb2xgnggmn6j4UaYccJYdo');
  });
});
