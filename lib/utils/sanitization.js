...
function sanitizeState(
  ...
  const keyserverInfosCopy: $ReadOnly<{ +[key: string]: KeyserverInfo }> =
    Object.fromEntries(
  ...
  const keyserverStore: KeyserverStore<KeyserverInfo> = {
    ...state.keyserverStore,
    keyserverInfos: keyserverInfosCopy,
  };

  const stateCopy = { ...state, keyserverStore };

  if (stateCopy.deviceToken !== undefined && stateCopy.deviceToken !== null) {
    stateCopy.deviceToken = null;
  }
  const stateCopy2 = clone(stateCopy);
  sanitizePII(stateCopy2, redactionHelpers);
  return stateCopy2;
}
