...
function isSuccessfulAuthType(action: BaseAction): boolean {
  const isAuthType: boolean =
...
function isSuccessfulLogin(action: BaseAction): boolean {
  if (
    action.type !== logInActionTypes.success ||
    action.type !== siweActionTypes.success
  ) {
  ...
  if (action.type === siweActionTypes.success) {
    isNewAccount = action.payload.isNewAccount;
  }
  const isLogin: boolean = action.type === logInActionTypes.success;
  return isLogin || !isNewAccount;
}
