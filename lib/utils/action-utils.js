...
import { idWithKeyserverInfoRegex } from './validation-utils.js';
...
const keyserverIDfromIDRegex = '[0-9]+(?=\\|)';

function extractKeyserverIDFromID(id: string): ?string {
  if (!id.match(idWithKeyserverInfoRegex)) {
    return null;
  }

  const result = id.match(keyserverIDfromIDRegex);
