...
const setCookieActionType = "SET_COOKIE";
function setCookie(
  dispatch: Dispatch,
  ...
  invariant(
    response.cookieChange &&
      response.cookieChange.threadInfos &&
      response.cookieChange.userInfos &&
      typeof response.cookieChange.cookieInvalidated === "boolean",
    "all server calls that return a new cookie should include a valid " +
      "cookieChange object in their payload",
  );
  if (response.cookieChange.cookieInvalidated) {
    dispatch({
      type: setCookieActionType,
      payload: {
        cookie: newCookie,
        threadInfos: response.cookieChange.threadInfos,
        userInfos: response.cookieChange.userInfos,
        cookieInvalidated: true,
        currentUserInfo: response.cookieChange.currentUserInfo,
      },
    });
  } else {
    dispatch({
...
function bindCookieAndUtilsIntoFetchJSON(
  ...
  const attemptToResolveInvalidation = async (newAnonymousCookie: ?string) => {
    ...
    const newFetchJSON = newValidCookie
      ? bindCookieAndUtilsIntoFetchJSON(dispatch, newValidCookie, urlPrefix)
      : null;
  ...
  return (async (endpoint: Endpoint, data: Object) => {
...
function bindServerCalls(
  ...
  return (
    stateProps: { cookie: ?string, urlPrefix: string },
