...
function setNewSession(dispatch: Dispatch, sessionChange: SessionChange) {
  dispatch({
...
async function fetchNewCookieFromNativeCredentials(
  ...
  const boundFetchJSON = async (
    ...
    const innerBoundSetNewSession = (sessionChange: SessionChange) => {
      newSessionChange = sessionChange;
      setNewSession(dispatch, sessionChange);
    };
    try {
