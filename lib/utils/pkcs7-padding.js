...
/**
 * Calculates length of the data padded with given padding configuration.
 *
 * @param {number} inputLength length of the unpadded input (in bytes)
 * @param paddingConfiguration padding configuration
 * @returns length of the padded input (in bytes)
 */
function calculatePaddedLength(
  inputLength: number,
  { blockSizeBytes, superblockSizeBlocks }: PaddingConfiguration = PKCS7_10KB,
): number {
  const pkcs7padding = blockSizeBytes - (inputLength % blockSizeBytes);
  const pkcs7PaddedLen = inputLength + pkcs7padding;

  const numBlocks = pkcs7PaddedLen / blockSizeBytes;
  const paddingBlocks =
    superblockSizeBlocks - (numBlocks % superblockSizeBlocks);

  return pkcs7PaddedLen + paddingBlocks * blockSizeBytes;
}
...
export { PKCS7_10KB, pad, unpad, calculatePaddedLength };
