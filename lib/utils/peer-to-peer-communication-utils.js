...
async function encryptAndSendMessageToPeer(
  ...
  try {
    ...
    if (
      ...
      // We optimistically try encrypting the message without checking if the
      // session has been created. This is okay because in case of missing
      // session, encryption will be retried after session creation.
      success = true;
