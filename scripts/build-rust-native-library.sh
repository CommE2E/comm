...
# available to XCode
cp "$(readlink "${PRJ_ROOT}"/target/${TARGET}/cxxbridge/native_rust_library/src/lib.rs.cc)" .
cp "$(readlink "${PRJ_ROOT}"/target/${TARGET}/cxxbridge/native_rust_library/src/lib.rs.h)" .
cp "$(readlink "${PRJ_ROOT}"/target/${TARGET}/cxxbridge/rust/cxx.h)" .
