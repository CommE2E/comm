...
# user_credentials_file="keyserver/secrets/user_credentials.json"
...
set_up_or_abort() {
  ...
  timestamp=$(date +%s)
  db_version_name="comm_bakup$timestamp"
  echo "backup db name: $db_version_name"

  #Create backup db and move all tables from comm
  mysql -u "$USER" -Bse "CREATE DATABASE $db_version_name;GRANT ALL ON $db_version_name"'.*'" TO comm@localhost;"
...
if grep -q '"usingIdentityCredentials":.*true' "$user_credentials_file"; then    
    if (mysql -u "$USER" -Bse "USE comm; SELECT * FROM metadata" 2>/dev/null | grep "db_version"); then
      echo "'usingIdentityCredentials' is set to true in user_credentials.json, and the db is set up."
    else
