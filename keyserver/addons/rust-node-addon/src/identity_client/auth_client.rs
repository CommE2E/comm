...
impl Interceptor for AuthLayer {
  fn call(&mut self, mut request: Request<()>) -> Result<Request<()>, Status> {
    ...
    metadata.append("user_id", self.user_id.parse_to_ascii()?);
    metadata.append("device_id", self.device_id.parse_to_ascii()?);
    metadata.append("access_token", self.access_token.parse_to_ascii()?);
