use grpc_clients::identity::protos::authenticated::{
  inbound_keys_for_user_response::Identifier, InboundKeysForUserRequest,
};
...
pub async fn get_inbound_keys_for_user_device(
  ...
  let (username, wallet_address) = match response.identifier {
    Some(Identifier::Username(u)) => (Some(u), None),
    Some(Identifier::WalletAddress(w)) => (None, Some(w)),
    None => (None, None),
  };
