...
function getOneTimeKeyValues(keyBlob: string): $ReadOnlyArray<string> {
  ...
  const keys: $ReadOnlyArray<string> = Object.values(content.curve25519);
  return keys;
}
...
async function retrieveAccountKeysSet(
  ...
  let prekey = null;
  for (const key in prekeyMap) {
    prekey = prekeyMap[key];
  }
...
async function markKeysAsPublished(account: OlmAccount) {
...
async function verifyUserLogin(): Promise<IdentityInfo> {
