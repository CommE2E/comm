...
async function validateRolePermissions() {
  ...
  for (const result of results) {
    ...
    // The case of a rule being edited is handled below.
    ...
    // discrepenices that could be linked to a specific user-surfaced
    // permission. This could be useful in manually parsing through the
    // script results to 'write off' discrepencies as user role edits.
    ...
    console.log(
      `Potential permission disrecepencies for role ${roleName} that ` +
        `could be linked back to user surfaced permissions (i.e. not an ` +
        `actual discrepency, but rather a user edited a role: \n`,
    );
