...
// of it's inner types
function generateConverterFromValidator<T: any, C>(
  validator: TType<T>,
  inputName: string,
  ...
  if (validator.meta.kind === 'interface') {
    ...
    for (const key in validator.meta.props) {
      const inner = generateConverterFromValidator(
        validator.meta.props[key],
        `${inputName}.${key}`,
        validatorToBeConverted,
        conversionExpressionString,
      );
  ...
  if (validator.meta.kind === 'union') {
    const unionValidator = (validator: any);
