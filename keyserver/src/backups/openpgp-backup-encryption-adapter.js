...
async function prepareOpenPGPEncryption(
  ...
  const homedir = await mkdtemp(path.join(os.tmpdir(), 'backup-gpg-'));
  const cleanup = async () => {
    await rm(homedir, { force: true, recursive: true });
  };

  try {
    await importArmoredPublicKey(homedir, filename, armoredPublicKey);
    const recipient = await getImportedRecipientFingerprint(homedir, filename);
    return { homedir, recipient, cleanup };
  } catch (error) {
    try {
      await cleanup();
    } catch (cleanupError) {
