    ...
    for (const notifInfo of usersToCollapsableNotifInfo[userID]) {
      ...
      if (androidVersionsToTokens) {
        for (const [
          ...
          const deliveryPromise = (async () => {
            const notificationsArray = await prepareAndroidNotification(
              {
                notifTexts,
                newRawMessageInfos: shimmedNewRawMessageInfos,
                threadID: threadInfo.id,
            ...
            const notificationDeviceTokenPairs = [...deviceTokens].map(
              (deviceToken, idx) => {
                return { deviceToken, notification: notificationsArray[idx] };
              },
            );
            return await sendAndroidNotification(notificationDeviceTokenPairs, {
              ...notificationInfo,
              codeVersion,
            });
          })();
...
async function prepareAPNsNotification(
  ...
  const notificationsToSend = notificationsWithMessageInfos.map(
    (notif, idx) => {
      const notifCopyWithMessageInfos = _cloneDeep(notif);
      if (
        notifCopyWithMessageInfos.length() <= apnMaxNotificationPayloadByteSize
      ) {
        return notifCopyWithMessageInfos;
      }
      const notifWithoutMessageInfos = notifications[idx];
      const copyWithoutMessageInfos = _cloneDeep(notifWithoutMessageInfos);
      if (
        copyWithoutMessageInfos.length() <= apnMaxNotificationPayloadByteSize
      ) {
        console.warn(
...
async function prepareAndroidNotification(
  ...
  if (shouldBeEncrypted) {
    ...
    return notificationsWithMessageInfos.map((notif, idx) =>
      evaluateAndSelectNotification(notifications[idx], notif),
    );
  }
  ...
  return cookieIDs.map(() => notificationToSend);
}
...
async function sendAPNsNotification(
  ...
  invariant(
    new Set(notifications.map(notif => notif.id)).size === 1,
...
async function sendAndroidNotification(
  ...
  const deviceTokens = notificationDeviceTokenPairs.map(
    pair => pair.deviceToken,
  );
...
async function updateBadgeCount(
  ...
  if (androidVersionsToTokens) {
    for (const [
      ...
      deliveryPromises.push(
        sendAndroidNotification(notificationDeviceTokenPairs, {
          source,
          dbID,
          userID,
          codeVersion,
