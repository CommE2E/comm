...
describe('Queued DM Operations queries', () => {
  ...
  it('should add new queued operation', () => {
    ...
    const addedOperation = operations.find(op => op.queue_key === 'entry123');
    expect(addedOperation).toBeDefined();
    expect(addedOperation?.operation_data).toContain('New entry');
  });
  ...
  it('should remove operations older than timestamp', () => {
    ...
    expect(remainingOperations[0].queue_key).toBe('msg101');
  });
  ...
  it('should clear operations for specific queue', () => {
    ...
    const threadOperations = operations.filter(
      op => op.queue_type === 'thread',
    );
    expect(threadOperations?.length).toBe(0);

    // Other operations should remain
    const queueKeys = operations.map(op => op.queue_key);
