...
// Raw C++ MessageEntity with vector medias field
export type RawMessageEntity = {
  +message: WebMessage,
  +medias: EmscriptenVector<Media>,
};

// JavaScript MessageEntity with converted array medias field
...
declare export class SQLiteQueryExecutor {
  ...
  replaceMessageStoreThreads(
    threads: EmscriptenVector<{ +id: string, +startReached: number }>,
    backupItem: boolean,
  ): void;
  removeMessageStoreThreads(EmscriptenVector<string>): void;
  getAllMessageStoreThreads(): EmscriptenVector<{
    +id: string,
    +startReached: number,
  }>;
