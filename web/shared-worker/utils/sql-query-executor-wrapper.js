...
class SQLiteQueryExecutorWrapper {
  ...
  replaceIntegrityThreadHashesFromMap(threadHashes: ThreadHashes): void {
    const clientDBThreadHashes = entries(threadHashes).map(
      ([id, threadHash]) => ({
        id,
        threadHash: threadHash.toString(),
      }),
    );
    this.replaceIntegrityThreadHashes(clientDBThreadHashes);
  }
