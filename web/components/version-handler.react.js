...
function MinVersionHandler(): null {
  ...
  const isClientVersionUnsupportedPrev = React.useRef(false);
  const { pushModal } = useModalContext();

  React.useEffect(() => {
    if (isClientVersionUnsupported && !isClientVersionUnsupportedPrev.current) {
      isClientVersionUnsupportedPrev.current = true;
