...
class InputStateContainer extends React.PureComponent<Props, State> {
    ...
    return {
      ...
      result: {
        ...
        pendingUpload: {
          ...
          loop: false,
          uriIsReal: false,
          blobHash: encryptionResult ? encryptionResult.sha256Hash : null,
  ...
  async blobServiceUpload(
    ...
    // TODO: fix this on Blob service side
    // The shash / is valid Base64 char but it's recognized
    // by Blob service as S3 path separator
    const sanitizedBlobHash = input.blobHash.replace(/\//g, '_');
    ...
    if (!blobAlreadyExists) {
      ...
      formData.append('blob_hash', sanitizedBlobHash);
