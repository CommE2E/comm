...
class InputStateContainer extends React.PureComponent<Props, State> {
  ...
  replyCallbacks: Array<(params: ReplyParameters) => void> = [];
  ...
  addReply = (params: ReplyParameters) => {
    this.replyCallbacks.forEach(addReplyCallback => addReplyCallback(params));
    this.setState({ reply: params });
  };

  addReplyListener = (callbackReply: (params: ReplyParameters) => void) => {
    this.replyCallbacks.push(callbackReply);
  };

  removeReplyListener = (callbackReply: (params: ReplyParameters) => void) => {
