...
async function getNotifsAccountWithOlmData(
  ...
  const {
    values: { notificationAccount, notificationAccountEncryptionKey },
  ...
  const encryptedOlmData: ?EncryptedData = queryResult[olmDataKey]
    ? assertWithValidator(queryResult[olmDataKey], encryptedAESDataValidator)
    : undefined;

  const olmDataEncryptionKey: ?CryptoKey | ?SubtleCrypto$JsonWebKey =
    queryResult[olmDataEncryptionKeyDBLabel]
      ? assertWithValidator(
          queryResult[olmDataEncryptionKeyDBLabel],
          extendedCryptoKeyValidator,
        )
