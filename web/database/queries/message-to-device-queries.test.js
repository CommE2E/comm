...
describe('Message to device queries', () => {
  ...
  it('should duplicate messages for given device', () => {
    const newDeviceID = 'newDeviceID';
    queryExecutor?.duplicateMessagesForDevice(device1, newDeviceID);

    const device1Messages = queryExecutor?.getAllMessagesToDevice(device1);
    expect(device1Messages).toStrictEqual(device1MessagesOrdered);

    const newMessages = device1MessagesOrdered.map(msg => ({
      ...msg,
      deviceID: newDeviceID,
    }));

    const newDeviceMessages =
      queryExecutor?.getAllMessagesToDevice(newDeviceID);
    expect(newDeviceMessages).toStrictEqual(newMessages);
  });
});
