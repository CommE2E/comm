...
function TypeaheadTooltip(props: TypeaheadTooltipProps): React.Node {
  ...
  React.useEffect(() => {
    const setter = inputState.setTypeaheadState;
    setter({
      close: actions.length > 0 ? close : null,
      accept: actions.length > 0 ? accept : null,
    });
  }, [close, accept, actions, inputState.setTypeaheadState]);
  ...
  const overlayClasses = classNames(css.suggestionsContainer, {
    [css.notVisible]: !isAnimation,
    [css.visible]: isAnimation,
  });
