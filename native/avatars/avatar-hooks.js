...
function useUploadProcessedMedia(): (
  media: MediaResult,
  shouldUploadMetadataToKeyserver: boolean,
  ...
  const uploadProcessedMultimedia: MediaResult => Promise<?UpdateUserAvatarRequest> =
    React.useCallback(
      async (processedMedia, shouldUploadMetadataToKeyserver) => {
        const useBlobService =
          !shouldUploadMetadataToKeyserver || useBlobServiceUploads;
        if (!useBlobService) {
        ...
        const { id, uri } = await callBlobServiceUpload({
          ...
          keyserverOrThreadID: shouldUploadMetadataToKeyserver
            ? authoritativeKeyserverID
            : null,
          callbacks: { blobServiceUploadHandler },
        });
        if (!shouldUploadMetadataToKeyserver) {
          return {
            type: 'thick_thread_image',
            blobURI: uri,
            thumbHash,
            encryptionKey,
          };
...
function useNativeUpdateThreadImageAvatar(): (
  ...
  const nativeUpdateThreadImageAvatar = React.useCallback(
    async (
      ...
      const shouldUploadMetadataToKeyserver = !threadTypeIsThick(
        threadInfo.type,
      );
      const imageAvatarUpdateRequest = await uploadSelectedMedia(
        selection,
