...
function ThreadAvatar(props: Props): React.Node {
  ...
  let userOrChannel;
  if (displayUserIDForThread) {
    userOrChannel = displayUser;
  } else if (communityInfo?.farcasterChannelID) {
    userOrChannel = { fcChannelID: communityInfo.farcasterChannelID };
  }

  const resolvedThreadAvatar = useResolvedThreadAvatar(
    avatarInfo,
    userOrChannel,
  );
