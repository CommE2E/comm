...
function TypeaheadTooltip(props: TypeaheadTooltipProps): React.Node {
  ...
  // We're adding PanGestureHandler so it handles scroll/pan events when
  // FlatList is not scrollable i.e when there are too few items in it
  // When that was a case on Android scroll events would be handled
  // by MessageList below it and it would be scrolled.
  // This way GestureHandler handles those events and they don't
  // cause unnecessary scrolling of list below overlay
  return (
    <PanGestureHandler>
      <FlatList
