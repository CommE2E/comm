...
import { useProtocolSelection } from 'lib/components/protocol-selection-provider.react';
...
import { useExistingThreadInfoFinder, threadIsPending } from 'lib/shared/thread-utils.js';
...
type Props = {
  ...
  // +updateSelectedProtocol: (?ProtocolName) => mixed,
...
const ConnectedMessageListContainer: React.ComponentType<BaseProps> =
  React.memo(function ConnectedMessageListContainer(props: BaseProps) {
    ...
    >([]);
    const { setUserInfoInput, selectedProtocol, setIsSearching, setIsThreadPending } = useProtocolSelection();
    React.useEffect(
      // We use it to sync the value
      () => {
        setUserInfoInput(userInfoInputArray);
        setIsThreadPending(threadIsPending(props.route.params.threadInfo.id));
      },
      [setUserInfoInput, userInfoInputArray, setIsThreadPending, props.route.params.threadInfo.id],
    );
