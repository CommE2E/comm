...
public class CommServicesAuthMetadataEmitter
  ...
  public static void
  sendAuthMetadataToJS(String accessToken, String userID, String deviceID) {
    synchronized (CommServicesAuthMetadataEmitter.class) {
      if (sharedInstance == null) {
        return;
      }

      // Event body must match IdentityServiceAuthLayer
      // type from 'lib/types/identity-service-types.js'
      WritableMap eventBody = Arguments.createMap();
      eventBody.putString("commServicesAccessToken", accessToken);
      eventBody.putString("userID", userID);
      eventBody.putString("deviceID", deviceID);
