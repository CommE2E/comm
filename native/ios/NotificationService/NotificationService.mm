...
- (void)didReceiveNotificationRequest:(UNNotificationRequest *)request
  ...
  std::unique_ptr<comm::NotificationsCryptoModule::CryptoModuleStateHolder>
      cryptoModuleStatePtr;
  ...
  if ([self shouldBeDecrypted:content.userInfo]) {
    ...
    try {
      @try {
        cryptoModuleStatePtr = [self decryptContentInPlace:content];
  ...
  if (decryptionExecuted) {
    comm::NotificationsCryptoModule::flushState(
        std::move(cryptoModuleStatePtr), callingProcessName);
  }
}
...
- (std::unique_ptr<comm::NotificationsCryptoModule::CryptoModuleStateHolder>)
  ...
  NSString *decryptedSerializedPayload =
      [NSString stringWithUTF8String:decryptResult.first.c_str()];
  ...
  return std::move(decryptResult.second);
}
