...
#import "NativeTextInputViewManager.h"
#import "LegacyNativeTextInputView.h"
#import "NativeTextInputShadowView.h"
...
@interface NativeTextInputViewManager () <RCTUIManagerObserver>

@end

@implementation NativeTextInputViewManager {
  NSHashTable<NativeTextInputShadowView *> *_shadowViews;
}

RCT_EXPORT_MODULE(NativeTextInputView)
...
- (UIView *)view
{
  return [[LegacyNativeTextInputView alloc] initWithBridge:self.bridge];
}

- (RCTShadowView *)shadowView
{
  NativeTextInputShadowView *shadowView = [[NativeTextInputShadowView alloc] initWithBridge:self.bridge];
...
RCT_EXPORT_METHOD(getValueAndReset
                    : (nonnull NSNumber *)viewTag resolver
                    : (RCTPromiseResolveBlock) resolve rejecter
                    : (RCTPromiseRejectBlock) reject)
{
  [self.bridge.uiManager addUIBlock:^(RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    LegacyNativeTextInputView *view = (LegacyNativeTextInputView *)viewRegistry[viewTag];
    NSString* value = [view getValueAndReset];
    resolve(value);
  }];
}
...
- (void)handleDidUpdateMultiplierNotification
{
  ...
  NSHashTable<NativeTextInputShadowView *> *shadowViews = _shadowViews;
  RCTExecuteOnUIManagerQueue(^{
    for (NativeTextInputShadowView *shadowView in shadowViews) {
