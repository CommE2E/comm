...
import BaseTooltipItem from './tooltip-item.react';
...
export type TooltipMenuProps<RouteName> = {
  ...
  +tooltipItem: React.ComponentType<RegisterOptionInput>,
};

function FixedTooltipItem(input: RegisterOptionInput): React.Node {
  return (
    <BaseTooltipItem
      {...input}
      containerStyle={[
        unboundStyles.itemContainer,
        unboundStyles.itemContainerFixed,
      ]}
    />
  );
}

function FloatingTooltipItem(input: RegisterOptionInput): React.Node {
  return (
    <BaseTooltipItem {...input} containerStyle={unboundStyles.itemContainer} />
  );
}
...
>(
  ...
  > {
    ...
    getTooltipItem() {
      if (this.tooltipLocation === 'fixed') {
        return FixedTooltipItem;
      } else {
        return FloatingTooltipItem;
      }
    }
  ...
  function ConnectedTooltip(props: BaseTooltipPropsType) {
    ...
    }, [actionSheetShown.value, goBackOnce, showEmojiKeyboard.value]);
