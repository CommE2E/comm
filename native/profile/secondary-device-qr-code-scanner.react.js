...
import { secondaryDeviceRegisteredAlertDetails } from '../utils/alert-messages.js';
...
function SecondaryDeviceQRCodeScanner(props: Props): React.Node {
  ...
  const tunnelbrokerMessageListener = React.useCallback(
    async (message: TunnelbrokerMessage) => {
      ...
      if (!payload.requestBackupKeys) {
        Alert.alert(
          secondaryDeviceRegisteredAlertDetails.title,
          secondaryDeviceRegisteredAlertDetails.message,
  ...
  const onPressSave = React.useCallback(async () => {
    ...
    const strippedUrl = urlInput.replace(/^comm:\/\/qr-code\//, '');

    try {
      console.log(decodeURIComponent(strippedUrl));
      const keys = JSON.parse(decodeURIComponent(strippedUrl));
