...
import { useCurrentUserFID, useLinkFIDDCs } from 'lib/utils/farcaster-utils.js';
...
function InnerConnectFarcasterDCs(props: Props): React.Node {
  ...
  const inputRef =
    React.useRef<?React.ElementRef<typeof RegistrationTextInput>>(null);

  const fid = useCurrentUserFID();
  const getAuthToken = useGetAuthToken();
  const linkFIDDCs = useLinkFIDDCs();
  ...
  }, [getAuthToken, linkFIDDCs, mnemonic, fid, onSuccess]);
