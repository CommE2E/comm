...
const FullScreenViewModal: React.ComponentType<Props> = React.memo<Props>(
  function ConnectedFullScreenViewModal(props: Props) {
    ...
    const panUpdate = React.useCallback(
      ...
      [curX, curY, lastPanTranslationX, lastPanTranslationY],
    );
    ...
    const isRunningResetAnimation = useSharedValue(false);
    ...
    const doubleTapUpdate = React.useCallback(
      ({ x, y }: TapGestureEvent) => {
        ...
        isRunningResetAnimation.value = true;
        curScale.value = withTiming(
          targetScale,
          defaultTimingConfig,
          () => (isRunningResetAnimation.value = false),
        );
    ...
    const contentViewContainerStyle = useAnimatedStyle(() => {
      ...
      const top = (dimFrameHeight - height) / 2 + topInset;
      const left = (dimFrameWidth - width) / 2;
