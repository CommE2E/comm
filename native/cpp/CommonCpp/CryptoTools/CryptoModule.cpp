...
size_t CryptoModule::publishOneTimeKeys() {
  ...
  folly::dynamic oneTimeKeysObj = folly::dynamic::object();
  folly::dynamic curve25519Keys = folly::dynamic::object();

  for (const auto &otk : oneTimeKeysVec) {
    std::string keyId = std::string(otk.key_id());
    std::string key = std::string(otk.key());
    curve25519Keys[keyId] = key;
  }

  oneTimeKeysObj["curve25519"] = curve25519Keys;
  this->keys.oneTimeKeys = folly::toJson(oneTimeKeysObj);
...
bool CryptoModule::prekeyDoesntExist() {
  ...
  return prekey == emptyPrekey || signature == emptySignature;
}
...
bool CryptoModule::isPrekeySignatureValid() {
  try {
    ...
    this->verifySignature(signingPublicKey, prekey, prekeySignature);
    return true;
