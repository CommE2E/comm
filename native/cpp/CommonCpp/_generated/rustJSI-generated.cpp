...
static jsi::Value __hostFunction_CommRustModuleSchemaCxxSpecJSI_logInSecondaryDevice(jsi::Runtime &rt, TurboModule &turboModule, const jsi::Value* args, size_t count) {
  return static_cast<CommRustModuleSchemaCxxSpecJSI *>(&turboModule)->logInSecondaryDevice(rt, args[0].asString(rt), args[1].asString(rt), args[2].asString(rt), args[3].asString(rt), args[4].asString(rt), args[5].asString(rt), args[6].asString(rt), args[7].asObject(rt).asArray(rt), args[8].asObject(rt).asArray(rt));
}
...
CommRustModuleSchemaCxxSpecJSI::CommRustModuleSchemaCxxSpecJSI(std::shared_ptr<CallInvoker> jsInvoker)
  ...
  methodMap_["logInSecondaryDevice"] = MethodMetadata {9, __hostFunction_CommRustModuleSchemaCxxSpecJSI_logInSecondaryDevice};
}
