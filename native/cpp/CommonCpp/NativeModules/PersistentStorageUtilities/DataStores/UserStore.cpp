...
jsi::Array UserStore::parseDBDataStore(
  ...
  for (const UserInfo &user : *usersVectorPtr) {
    ...
    jsiUser.setProperty(
        rt,
        "username",
        user.username ? jsi::String::createFromUtf8(rt, *user.username)
                      : jsi::Value::null());
    jsiUser.setProperty(
        rt,
        "relationshipStatus",
        user.relationship_status
            ? jsi::String::createFromUtf8(rt, *user.relationship_status)
            : jsi::Value::null());
    jsiUser.setProperty(
        rt,
        "avatar",
        user.avatar ? jsi::String::createFromUtf8(rt, *user.avatar)
                    : jsi::Value::null());
...
UserStore::createOperations(jsi::Runtime &rt, const jsi::Array &operations)
  ...
  for (size_t idx = 0; idx < operations.size(rt); idx++) {
    ...
    if (opType == REMOVE_OPERATION) {
      ...
      jsi::Value maybeUserName = userObj.getProperty(rt, "username");
      std::unique_ptr<std::string> username = maybeUserName.isString()
          ? std::make_unique<std::string>(maybeUserName.asString(rt).utf8(rt))
          : nullptr;

      jsi::Value maybeRelationshipStatus =
          userObj.getProperty(rt, "relationshipStatus");
      std::unique_ptr<std::string> relationshipStatus =
          maybeRelationshipStatus.isString()
          ? std::make_unique<std::string>(
                maybeRelationshipStatus.asString(rt).utf8(rt))
          : nullptr;

      jsi::Value maybeAvatar = userObj.getProperty(rt, "avatar");
