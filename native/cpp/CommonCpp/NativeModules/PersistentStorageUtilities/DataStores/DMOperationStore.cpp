...
OperationType DMOperationStore::ADD_QUEUED_OPERATION =
    "add_queued_dm_operation";
OperationType DMOperationStore::CLEAR_QUEUED_OPERATION =
    "clear_dm_operations_queue";
OperationType DMOperationStore::PRUNE_QUEUED_OPERATION =
    "prune_queued_dm_operations";
...
DMOperationStore::createOperations(
  ...
  for (size_t idx = 0; idx < operations.size(rt); idx++) {
    ...
    if (opType == REMOVE_OPERATION) {
    ...
    } else if (opType == ADD_QUEUED_OPERATION) {
      jsi::Object payloadObj = op.getProperty(rt, "payload").asObject(rt);
      ...
      dmOperationStoreOps.push_back(
          std::make_unique<AddQueuedDMOperationOperation>(
              std::move(operation)));
    } else if (opType == CLEAR_QUEUED_OPERATION) {
      jsi::Object payloadObj = op.getProperty(rt, "payload").asObject(rt);
      std::string queueType =
          payloadObj.getProperty(rt, "queueType").asString(rt).utf8(rt);
      std::string queueKey =
      ...
      dmOperationStoreOps.push_back(
          std::make_unique<ClearQueuedDMOperationsOperation>(
              queueType, queueKey));
    } else if (opType == PRUNE_QUEUED_OPERATION) {
      jsi::Object payloadObj = op.getProperty(rt, "payload").asObject(rt);
      std::string timestamp =
          payloadObj.getProperty(rt, "timestamp").asString(rt).utf8(rt);
