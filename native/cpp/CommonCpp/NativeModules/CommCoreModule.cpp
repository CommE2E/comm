...
void CommCoreModule::persistCryptoModules(
  ...
  GlobalDBSingleton::instance.scheduleOrRunCancellable(
      [=, &persistencePromise, &maybeUpdatedNotifsCryptoModule]() {
        try {
          DatabaseManager::getQueryExecutor().beginTransaction();
          if (persistContentModule) {
  ...
  return createPromiseAsJSIValue(
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=]() {
          ...
          std::string error;
          bool notifsCryptoAccountPresentInMMKV = false;
          try {
            ...
            if (NotificationsCryptoModule::
              ...
              notifsCryptoAccountPresentInMMKV = true;
            } else {
              notifsAccountData =
                  DatabaseManager::getQueryExecutor().getOlmPersistAccountData(
          ...
          this->cryptoThread->scheduleTask([=]() {
            ...
            if (!notifsCryptoAccountPresentInMMKV) {
              maybeNotifsCryptoAccountToPersist = {
      ...
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=, &innerRt]() {
          ...
          try {
            ...
            this->persistCryptoModules(
                true, std::move(notifsCryptoModuleWithPicklingKey));
          } catch (const std::exception &e) {
            error = e.what();
          }
          this->jsInvoker_->invokeAsync([=, &innerRt]() {
...
jsi::Value CommCoreModule::isContentSessionInitialized(
  ...
  return createPromiseAsJSIValue(
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=, &innerRt]() {
          bool hasSession =
              this->contentCryptoModule->hasSessionFor(deviceIDCpp);
          this->jsInvoker_->invokeAsync(
              [=, &innerRt]() { promise->resolve(hasSession); });
        };
        this->cryptoThread->scheduleTask(job);
      });
}
