      ...
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=, &innerRt]() {
          ...
          if (this->cryptoModule == nullptr) {
            ...
            try {
              std::optional<std::string> contentPrekeyOpt =
                  this->cryptoModule->validatePrekey();
              if (contentPrekeyOpt.has_value()) {
                this->persistCryptoModule();
              } else {
                contentPrekeyOpt = this->cryptoModule->getUnpublishedPrekey();
              }
              if (contentPrekeyOpt.has_value()) {
                contentPrekey = contentPrekeyOpt.value();
              } else {
                contentPrekey = this->cryptoModule->getPrekey();
              }

              contentPrekeySignature = this->cryptoModule->getPrekeySignature();
              std::tuple<std::string, std::string> notifPrekeyAndSignature =
                  getNotificationsPrekeyAndSignature();
              notifPrekey = std::get<0>(notifPrekeyAndSignature);
              notifPrekeySignature = std::get<1>(notifPrekeyAndSignature);
            } catch (const std::exception &e) {
              error = e.what();
            }
          }
