...
jsi::Value
CommCoreModule::updateDataInNotifStorage(jsi::Runtime &rt, jsi::Array data) {
...
jsi::Value
CommCoreModule::getDataFromNotifStorage(jsi::Runtime &rt, jsi::Array ids) {
  std::vector<std::string> idsCpp{};
  for (auto idx = 0; idx < ids.size(rt); idx++) {
    std::string id = ids.getValueAtIndex(rt, idx).asString(rt).utf8(rt);
    idsCpp.push_back(id);
  }
      ...
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        ...
        this->jsInvoker_->invokeAsync(
            [&innerRt, dataVectorPtr, error, promise]() {
              ...
              jsi::Array jsiData = jsi::Array(innerRt, numData);
              size_t writeIdx = 0;

              for (const auto &dataItem : *dataVectorPtr) {
                jsi::Object jsiDataItem = jsi::Object(innerRt);
                jsiDataItem.setProperty(innerRt, "id", dataItem.first);
                jsiDataItem.setProperty(
                    innerRt, "unreadCount", dataItem.second);
                jsiData.setValueAtIndex(innerRt, writeIdx++, jsiDataItem);
              }
...
jsi::Value
CommCoreModule::getDatabaseVersion(jsi::Runtime &rt, jsi::String dbID) {
  ...
  return createPromiseAsJSIValue(
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=, &innerRt]() {
          ...
          int version = 0;
          try {
            version = DatabaseManager::getQueryExecutor(identifier)
                          .getDatabaseVersion();
          } catch (std::system_error &e) {
            error = e.what();
          }
...
jsi::Value CommCoreModule::getHolders(jsi::Runtime &rt, jsi::String dbID) {
  DatabaseIdentifier identifier = stringToDatabaseIdentifier(dbID.utf8(rt));
  return createPromiseAsJSIValue(
      rt, [=](jsi::Runtime &innerRt, std::shared_ptr<Promise> promise) {
        taskType job = [=, &innerRt]() {
          ...
          this->jsInvoker_->invokeAsync([&innerRt,
            ...
            jsi::Array jsiHolders =
                holderStore.parseDBDataStore(innerRt, holdersVectorPtr);
            promise->resolve(std::move(jsiHolders));
          });
        };
        GlobalDBSingleton::instance.scheduleOrRunCancellable(
            job, promise, this->jsInvoker_);
