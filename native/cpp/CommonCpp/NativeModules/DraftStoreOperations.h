...
public:
  ...
  virtual void execute(std::string db) override {
    DatabaseManager::getQueryExecutor(db).updateDraft(this->key, this->text);
  }
...
public:
  ...
  virtual void execute(std::string db) override {
    DatabaseManager::getQueryExecutor(db).moveDraft(this->oldKey, this->newKey);
  }
...
public:
  virtual void execute(std::string db) override {
    DatabaseManager::getQueryExecutor(db).removeAllDrafts();
  }
};
...
public:
  ...
  virtual void execute(std::string db) override {
    DatabaseManager::getQueryExecutor(db).removeDrafts(this->idsToRemove);
  }
