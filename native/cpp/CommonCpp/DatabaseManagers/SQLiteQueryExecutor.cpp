...
bool create_keyservers_synced_data(sqlite3 *db) {
  std::string query =
      "CREATE TABLE IF NOT EXISTS keyservers_synced_data ("
      "	 id TEXT UNIQUE PRIMARY KEY NOT NULL,"
      "	 keyserver_info TEXT NOT NULL"
      ");";
  bool success = create_table(db, query, "keyservers_synced_data");
  if (!success) {
    return false;
  }

  std::string copyData =
      "INSERT INTO keyservers_synced_data (id, keyserver_info)"
...
std::vector<std::pair<unsigned int, SQLiteMigration>> migrations{
    {{1, {create_drafts_table, true}},
     ...
     {39, {create_keyservers_synced_data, true}}}};
...
void SQLiteQueryExecutor::replaceKeyserver(
  ...
  static std::string replaceKeyserverSyncedSQL =
      "REPLACE INTO keyservers_synced_data (id, keyserver_info) "
...
void SQLiteQueryExecutor::removeAllKeyservers() const {
  ...
  static std::string removeAllKeyserversSyncedSQL =
      "DELETE FROM keyservers_synced_data;";
  removeAllEntities(
...
void SQLiteQueryExecutor::removeKeyservers(
  ...
  removeKeyserversSyncedByKeysSQLStream << "DELETE FROM keyservers_synced_data "
                                           "WHERE id IN "
                                        << idArray << ";";
...
std::vector<KeyserverInfo> SQLiteQueryExecutor::getAllKeyservers() const {
  static std::string getAllKeyserversSQL =
      "SELECT synced.id,"
      " COALESCE(keyservers.keyserver_info, ''), synced.keyserver_info"
      " FROM keyservers_synced_data synced"
