...
std::vector<std::pair<unsigned int, SQLiteMigration>> migrations {
  {
    {1, {create_drafts_table, true}}, {2, {rename_threadID_to_key, true}},
        {4, {create_persist_account_table, true}},
        {5, {create_persist_sessions_table, true}},
        {15, {create_media_table, true}}, {16, {drop_messages_table, true}},
        ...
        {32, {create_users_table, true}}, {33, {create_keyservers_table, true}},
    ...
    MigrationResult applyMigrationWithTransaction(
        sqlite3 * db, const MigrateFunction &migrate, int index) {
...
// https://linear.app/comm/issue/ENG-6398/issues-with-sqlcipher-on-web
      ...
      for (const auto &[idx, migration] : migrations) {
        ...
        if (shouldBeInTransaction) {
          migrationResult =
              applyMigrationWithTransaction(db, applyMigration, idx);
        } else {
          migrationResult =
    ...
    std::unique_ptr<Thread> SQLiteQueryExecutor::getThread(std::string threadID)
        const {
      ...
      return getEntityByPrimaryKey<Thread>(
          ...
          getThreadByPrimaryKeySQL,
          threadID);
    }

    void SQLiteQueryExecutor::updateDraft(std::string key, std::string text)
        const {
    ...
    void SQLiteQueryExecutor::removeAllDrafts() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllDraftsSQL);
    }

    void SQLiteQueryExecutor::removeDrafts(const std::vector<std::string> &ids)
        const {
      if (!ids.size()) {
        return;
      }
    ...
    void SQLiteQueryExecutor::removeAllMessages() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllMessagesSQL);
    }
    ...
    SQLiteQueryExecutor::getAllMessages() const {
      ...
      for (int stepResult = sqlite3_step(preparedSQL); stepResult == SQLITE_ROW;
        ...
        if (message.id == prevMsgIdx) {
          allMessages.back().second.push_back(
          ...
          allMessages.push_back(
              std::make_pair(std::move(message), mediaForMsg));
        }
      }
      return allMessages;
    }
    ...
    void SQLiteQueryExecutor::rekeyMessage(std::string from, std::string to)
        const {
    ...
    void SQLiteQueryExecutor::removeAllMedia() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllMediaSQL);
    }
    ...
    void SQLiteQueryExecutor::removeAllMessageStoreThreads() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(),
          removeAllMessageStoreThreadsSQL);
    }
    ...
    void SQLiteQueryExecutor::removeThreads(std::vector<std::string> ids)
        const {
      if (!ids.size()) {
        return;
      }
    ...
    void SQLiteQueryExecutor::replaceThread(const Thread &thread) const {
      static std::string replaceThreadSQL =
          ...
          " id, type, name, description, color, creation_time, "
          "parent_thread_id,"
    ...
    void SQLiteQueryExecutor::removeAllThreads() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllThreadsSQL);
    };
    ...
    void SQLiteQueryExecutor::removeAllReports() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllReportsSQL);
    }

    void SQLiteQueryExecutor::removeReports(const std::vector<std::string> &ids)
        const {
      if (!ids.size()) {
        return;
      }
    ...
    void SQLiteQueryExecutor::setPersistStorageItem(
        std::string key, std::string item) const {
    ...
    std::string SQLiteQueryExecutor::getPersistStorageItem(std::string key)
        const {
    ...
    void SQLiteQueryExecutor::removeAllUsers() const {
      ...
      removeAllEntities(
          SQLiteQueryExecutor::getConnection(), removeAllUsersSQL);
    }

    void SQLiteQueryExecutor::removeUsers(const std::vector<std::string> &ids)
        const {
      if (!ids.size()) {
        return;
      }
    ...
    std::optional<std::string> SQLiteQueryExecutor::getOlmPersistAccountData()
        const {
    ...
    void SQLiteQueryExecutor::storeOlmPersistData(crypto::Persist persist)
        const {
      std::string accountData =
          std::string(persist.account.begin(), persist.account.end());

      for (auto it = persist.sessions.begin(); it != persist.sessions.end();
           it++) {
    ...
    void SQLiteQueryExecutor::setMetadata(
        std::string entry_name, std::string data) const {
    ...
    std::vector<MessageWithMedias> SQLiteQueryExecutor::getAllMessagesWeb()
        const {
    ...
    void SQLiteQueryExecutor::replaceMessageWeb(const WebMessage &message)
        const {
    ...
    void SQLiteQueryExecutor::initialize(std::string & databasePath) {
      std::call_once(SQLiteQueryExecutor::initialized, [&databasePath]() {
        ...
        folly::Optional<std::string> maybeEncryptionKey = CommSecureStore::get(
            SQLiteQueryExecutor::secureStoreEncryptionKeyID);
    ...
    void SQLiteQueryExecutor::restoreFromMainCompaction(
        std::string mainCompactionPath, std::string mainCompactionEncryptionKey)
        const {

      if (!file_exists(mainCompactionPath)) {
        throw std::runtime_error(
            "Restore attempt but backup file does not exist.");
      }
...
// https://linear.app/comm/issue/ENG-6398/issues-with-sqlcipher-on-web
      ...
      if (backupResult == SQLITE_BUSY || backupResult == SQLITE_LOCKED) {
        throw std::runtime_error(
            "Programmer error. Database in transaction during restore "
            "attempt.");
      } else if (backupResult != SQLITE_DONE) {
        std::stringstream error_message;
