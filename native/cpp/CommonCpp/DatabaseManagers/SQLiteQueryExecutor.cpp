...
std::vector<std::pair<unsigned int, SQLiteMigration>> migrations{
    {{1, {create_drafts_table, true}},
     ...
     {36, {create_messages_to_device_table, false}}}};
...
SQLiteQueryExecutor::getAllMessagesToDevice(const std::string &deviceID) const {
  ...
  if (result != SQLITE_OK) {
    throw std::runtime_error(
        "Failed to prepare getAllMessagesToDevice statement: " +
  ...
  for (int stepResult = sqlite3_step(stmt); stepResult == SQLITE_ROW;
       stepResult = sqlite3_step(stmt)) {
    messages.emplace_back(
        ClientMessageToDevice(MessageToDevice::fromSQLResult(stmt, 0)));
  }
...
void SQLiteQueryExecutor::removeMessagesToDeviceOlderThan(
  ...
  if (result != SQLITE_OK) {
    throw std::runtime_error(
        "Failed to prepare removeMessagesToDeviceOlderThan statement: " +
        std::string(sqlite3_errmsg(SQLiteQueryExecutor::getConnection())));
  }

  sqlite3_bind_int64(stmt, 1, lastConfirmedMessage.timestamp);
  sqlite3_bind_text(
      stmt, 2, lastConfirmedMessage.device_id.c_str(), -1, SQLITE_TRANSIENT);
