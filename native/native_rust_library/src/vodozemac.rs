...
pub fn encrypt_result_new(encrypted_message: String, message_type: u32) -> Box<EncryptResult> {
...
impl TryFrom<&EncryptResult> for olm::OlmMessage {
  ...
  fn try_from(message: &EncryptResult) -> Result<Self, Self::Error> {
    match message.message_type {
      0 => {
        let prekey = olm::PreKeyMessage::from_base64(&message.encrypted_message)?;
        Ok(prekey.into())
      }
      1 => {
...
impl InboundCreationResult {
  ...
  pub fn take_session(&mut self) -> Box<VodozemacSession> {
    Box::new(
      self.session.take()
        .expect("Session has already been taken")
    )
  }
}
...
impl VodozemacAccount {
  ...
  pub fn one_time_keys(&self) -> Vec<OneTimeKey> {
    self.0
        .one_time_keys()
        .into_iter()
        .map(|(k, v)| OneTimeKey {key_id: k.to_base64(), key: v.to_base64()})
        .collect()
  }
  ...
  pub fn prekey(&self) -> String {
    self.0.prekey().map(|key| key.to_base64()).unwrap_or_default()
  }

  pub fn unpublished_prekey(&self) -> String {
    self.0.unpublished_prekey().map(|key| key.to_base64()).unwrap_or_default()
  }
  ...
  pub fn create_outbound_session(
    ...
    let identity_key =
        vodozemac::Curve25519PublicKey::from_base64(identity_key)
          .map_err(|e| e.to_string())?;
    let signing_key =
        vodozemac::Ed25519PublicKey::from_base64(signing_key)
          .map_err(|e| e.to_string())?;
  ...
  pub fn create_inbound_session(
    ...
    let identity_key =
        vodozemac::Curve25519PublicKey::from_base64(identity_key)
          .map_err(|e| e.to_string())?;
    let olm_message: olm::OlmMessage = message.try_into()
