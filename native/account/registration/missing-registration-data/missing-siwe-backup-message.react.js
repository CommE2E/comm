...
function CreateMissingSIWEBackupMessage(props: Props): React.Node {
  ...
  invariant(
    ...
    'current username must be valid ethereum address to attempt backup message generation.',
  );
    ...
    (result: SIWEResult) => {
      void (async () => {
        ...
        if (loggedInEthereumAccountAddress !== address) {
          Alert.alert(
            'Change in Ethereum account',
            'Please make sure you use the same Ethereum address for signature generation.',
          );
          return;
        }
