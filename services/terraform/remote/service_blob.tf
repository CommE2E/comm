locals {
  ...
  blob_service_server_image        = "commapp/blob-server:latest"
...
resource "aws_ecs_task_definition" "blob_service" {
  ...
  task_role_arn            = data.aws_iam_role.services_ddb_full_access.arn
...
resource "aws_lb_listener" "blob_service_https" {
  ...
  default_action {
    ...
    forward {
      ...
      # Legacy EC2 Target
      target_group {
        arn    = data.aws_lb_target_group.blob_service_legacy_ec2.arn
        weight = 0
      }
    }
  }
