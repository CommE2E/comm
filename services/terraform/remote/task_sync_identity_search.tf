...
resource "aws_iam_role" "sync_identity_search_scheduler" {
  name = "cron-sync-identity-search-scheduler-role"
...
resource "aws_iam_role_policy_attachment" "sync_identity_search_scheduler" {
  ...
  role       = aws_iam_role.sync_identity_search_scheduler.name
}
