...
use chrono::Utc;
use comm_lib::{
  aws::ddb::types::{AttributeValue, ReturnValue},
  database::TryFromAttribute,
};
...
impl DatabaseClient {
  pub async fn insert_workflow(
    ...
    let workflow_expiration_time =
      Utc::now() + WORKFLOWS_IN_PROGRESS_TTL_DURATION;
  ...
  pub async fn get_workflow(
    ...
    // We call DeleteItem to ensure that the workflow is retrieved only once
    ...
    let mut workflow_item = delete_response.attributes.unwrap_or_default();
    let raw_workflow =
