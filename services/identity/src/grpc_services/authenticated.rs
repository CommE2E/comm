...
impl IdentityClientService for AuthenticatedService {
  ...
  async fn log_out_user(
    ...
    device_list.device_ids.into_iter().for_each(|device_id| {
      tokio::spawn(async move {
        debug!("Sending device list updates to {}", device_id);
        let result =
          crate::tunnelbroker::send_device_list_update(&device_id).await;
