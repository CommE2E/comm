...
struct PakeLoginResponseStructAndServerLogin {
  pake_login_response: PakeLoginResponseStruct,
...
) -> Result<PakeLoginResponseStructAndServerLogin, Status> {
  if user_id.is_empty() {
  ...
  match ServerLogin::start(
    ...
    Ok(server_login_start_result) => {
      Ok(PakeLoginResponseStructAndServerLogin {
        ...
        server_login: server_login_start_result.state,
      })
    }
    Err(e) => {
      error!(
  ...
  pub async fn handle_registration_upload_and_credential_request(
    ...
    match message {
      Some(Ok(RegistrationRequest {
        ...
        let pake_login_response_and_server_login =
        ...
        let registration_response = RegistrationResponse {
          data: Some(PakeRegistrationLoginResponse(
            pake_login_response_and_server_login.pake_login_response,
          )),
        };
        if let Err(e) = tx.send(Ok(registration_response)).await {
          ...
          Ok(pake_login_response_and_server_login.server_login)
        }
      }
      None | Some(_) => Err(Status::aborted("failure")),
    }
  ...
  pub async fn handle_login_request(
    ...
    match message {
      ...
      Some(Ok(LoginRequest {
        ...
        let pake_login_response_struct_and_server_login = pake_login_start(
          config.clone(),
          client.clone(),
        ...
        let login_response = LoginResponse {
          data: Some(PakeLoginResponse(
            pake_login_response_struct_and_server_login.pake_login_response,
          )),
        };
        ...
        Ok(Some(LoginState {
          ...
          pake_state: pake_login_response_struct_and_server_login.server_login,
        }))
      }
      Some(_) | None => Err(Status::aborted("failure")),
    }
