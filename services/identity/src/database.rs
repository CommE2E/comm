...
fn parse_devices_attribute(
  ...
  match attribute {
    Some(AttributeValue::M(ddb_devices_map)) => {
      for (signing_public_key, session_initialization_info) in ddb_devices_map {
        let session_initialization_info_map = parse_initialization_info(
...
fn parse_initialization_info(
  attribute_name: &'static str,
  attribute_value: AttributeValue,
) -> Result<HashMap<String, AttributeValue>, DBItemError> {
  match attribute_value {
    AttributeValue::M(initialization_info) => Ok(initialization_info),
    _ => Err(DBItemError::new(
      attribute_name,
