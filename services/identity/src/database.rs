...
impl DatabaseClient {
    ...
    loop {
      ...
      match transaction {
        ...
        Err(e) => {
          ...
          let retryable_codes =
            vec![retry::CONDITIONAL_CHECK_FAILED, retry::TRANSACTION_CONFLICT]
              .into_iter()
              .collect();
  ...
  pub async fn append_one_time_prekeys(
    ...
    let update_content_otk_count_operation = into_one_time_update_requests(
      &user_id,
      &device_id,
      num_content_keys,
      OlmAccountType::Content,
    ...
    let update_notif_otk_count_operation = into_one_time_update_requests(
      &user_id,
      &device_id,
      num_notif_keys,
      OlmAccountType::Notification,
    ...
    loop {
      ...
      match transaction {
        ...
        Err(e) => {
          let dynamo_db_error = DynamoDBError::from(e);
          let retryable_codes =
            vec![retry::TRANSACTION_CONFLICT].into_iter().collect();
          if is_transaction_retryable(&dynamo_db_error, &retryable_codes) {
