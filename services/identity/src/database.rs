...
impl DatabaseClient {
  ...
  pub async fn mint_onetime_key(
    ...
    loop {
      let query_items = self.get_onetime_key(device_id, account_type).await?;

      let first_item = query_items
        .items
        .map(|x| x.into_iter().next())
        .unwrap_or(None);

      let item = match first_item {
        Some(item) => item,
        None => {
      ...
      let Err(failure) = result else {
        return Ok(Some(otk.to_string()));
      };

      warn!("Failed to delete onetime key: {:?}", failure);
    }
  }
  ...
  async fn add_device_to_users_table(
    ...
    // BatchWriteItem only supports 25 writes in a call
    for requests in otk_requests.chunks(25) {
      self
        .client
        .batch_write_item()
