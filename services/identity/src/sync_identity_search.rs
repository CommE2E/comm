...
use crate::constants::{IDENTITY_SEARCH_INDEX, SYNC_LOG_ERROR_TYPE};
...
pub async fn clear_index(
  ...
  if !response.status().is_success() {
    error!(errorType = SYNC_LOG_ERROR_TYPE, "Failed to clear index");
  }
...
pub async fn restore_index(
  ...
  if !response.status().is_success() {
    error!(errorType = SYNC_LOG_ERROR_TYPE, "Failed to restore index");
  }
