...
impl IdentityClientService for ClientService {
  ...
  async fn restore_user(
    ...
    let request_has_user_keys =
      !new_backup_id.is_empty() && !encrypted_user_keys.is_empty();
    ...
    let (backup_item, revoke) = if request_has_user_keys {
      ...
      error!(
        errorType = error_types::GRPC_SERVICES_LOG,
        "RestoreUser request without UserKeys present. This shouldn't happen."
      );
      (None, Defer::empty())
    };
  ...
  async fn upload_keys_for_registered_device_and_log_in(
    ...
    info!(
      user_id = redact_sensitive_data(&message.user_id),
      "Attempting to authenticate a new secondary device.",
    );
...
impl ClientService {
  ...
  async fn restore_primary_device_and_get_csat(
    ...
    let excluded_device_ids =
      if let Some(ref keyserver_device_id) = maybe_keyserver_device_id {
        vec![keyserver_device_id]
      } else {
        vec![]
      };
