...
async fn test_device_list_updates() {
  use commtest::identity::olm_account_infos::{
    DEFAULT_CLIENT_KEYS as DEVICE_KEYS_A, MOCK_CLIENT_KEYS_1 as DEVICE_KEYS_B,
    MOCK_CLIENT_KEYS_2 as DEVICE_KEYS_C,
  };
  ...
  //
  let android_device_id = &DEVICE_KEYS_A.primary_identity_public_keys.ed25519;
  let web_device_id = &DEVICE_KEYS_B.primary_identity_public_keys.ed25519;
  let ios_device_id = &DEVICE_KEYS_C.primary_identity_public_keys.ed25519;

  // 1. Register user with primary Android device
  let android =
    register_user_device(Some(&DEVICE_KEYS_A), Some(DeviceType::Android)).await;

  // 2. Login a web device
  let web = login_user_device(
    &android.username,
    Some(&DEVICE_KEYS_B),
    Some(DeviceType::Web),
  )
  ...
  // 4. Login an iOS device
  let ios = login_user_device(
    &web.username,
    Some(&DEVICE_KEYS_C),
    Some(DeviceType::Ios),
  )
