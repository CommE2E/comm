    ...
    let outbound = async_stream::stream! {
      while let Some(data) = request_thread_rx.recv().await {
        ...
        if let Some (unpacked_data) = request_data {
          ...
          report_error(
            &ERROR_MESSAGES, 
            "an error occured, aborting connection", Some("put")
          );
          break;
        }
      }
  ...
  RUNTIME.block_on(async {
    if let Ok(mut clients) = CLIENTS.lock() {
      ...
      if let Some(client) = maybe_client {
        ...
        report_error(&ERROR_MESSAGES, "no client detected 1", Some("put"));
      }
    } else {
      report_error(&ERROR_MESSAGES, "couldn't access client", Some("put"));
    }
  ...
  RUNTIME.block_on(async {
    if let Ok(clients) = CLIENTS.lock() {
      ...
      if let Some(client) = maybe_client {
        ...
        report_error(&ERROR_MESSAGES, "no client detected 2", Some("put"));
      }
    } else {
      report_error(&ERROR_MESSAGES, "couldn't access client", Some("put"));
    }
...
pub fn put_client_terminate_cxx(
  ...
  if let Ok(mut clients) = CLIENTS.lock() {
    ...
    if let Some(client) = maybe_client {
      ...
      return Err("no client detected 3".to_string());
    }
  } else {
