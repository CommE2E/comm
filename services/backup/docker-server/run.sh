...
then
  ...
  then
    ...
    docker build -t $IMAGE_NAME:$VERSION -f services/backup/docker-server/Dockerfile .
    docker run -it -p 50051:50051 -v $HOME/.aws/credentials:/root/.aws/credentials:ro $IMAGE_NAME:$VERSION
  else
