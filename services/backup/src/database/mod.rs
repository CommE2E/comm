...
impl DatabaseClient {
  ...
  pub async fn remove_old_backups(
    ...
    let mut items = response.items.unwrap_or_default();
    // Skipping latest
    items.remove(0);

    let mut backups = vec![];
    for item in items {
      let ordered_backup = OrderedBackupItem::try_from(item)?;

      trace!("Removing backup item: {ordered_backup:?}");

      if let Some(backup) = self
        .remove_backup_item(user_id, &ordered_backup.backup_id)
        .await?
      {
        backups.push(backup);
      } else {
