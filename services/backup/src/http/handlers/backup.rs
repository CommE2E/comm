...
async fn get_total_backup_size(
  ...
  if let Some(user_data) = backup_item.user_data {
    blob_infos.push(user_data);
  }

  // gather blob infos and DDB size for logs
  let (mut log_blob_infos, ddb_logs_size) = db_client
    .get_blob_infos_and_size_for_logs(user_id, &backup_item.backup_id)
    .await?;
  blob_infos.append(&mut log_blob_infos);
