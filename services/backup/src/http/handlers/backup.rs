...
pub async fn upload(
  ...
  let Some((name, backup_id)) = get_text_field(&mut multipart).await? else {
    warn!("Malfolmed request: expected a field.");
    return Err(ErrorBadRequest("Bad request"));
  };
  if name != "backup_id" {
    warn!(name, "Malfolmed request: 'backup_id' text field expected.");
    return Err(ErrorBadRequest("Bad request"));
  }
    ...
    match get_text_field(&mut multipart).await? {
      Some((name, attachments)) => {
        if name != "attachments" {
          warn!(
            name,
            "Malfolmed request: 'attachments' text field expected."
          );
          return Err(ErrorBadRequest("Bad request"));
        }
...
async fn forward_field_to_blob(
  ...
  if name != hash_field_name {
    warn!(
      name,
      "Malfolmed request: '{hash_field_name}' text field expected."
    );
    return Err(ErrorBadRequest("Bad request"))?;
  }
  ...
  if field.name() != data_field_name {
    warn!(
      name,
      "Malfolmed request: '{data_field_name}' text field expected."
    );
    return Err(ErrorBadRequest("Bad request"))?;
  }
  ...
  let receive_promise = async move {
    ...
    // [`actix_multipart::MultipartError`] isn't [`std::marker::Send`] to we return it here, and pass [`Infallible`]
