...
pub mod batch_operations {
  ...
  #[tracing::instrument(
    name = "batch_write",
    skip(ddb, items, config),
    fields(thread)
  )]
  pub async fn batch_write(
    ddb: &aws_sdk_dynamodb::Client,
    table_name: &str,
    items: Vec<super::AttributeMap>,
    config: ExponentialBackoffConfig,
  ) -> Result<(), super::Error> {
    tracing::Span::current()
      .record("thread", format!("{:?}", std::thread::current().id()));
    ...
    let exponential_backoff_enabled = config.max_attempts > 0;
    let mut exponential_backoff = ExponentialBackoffHelper::new(&config);
    ...
    loop {
      ...
      if exponential_backoff_enabled
        && config.retry_on_provisioned_capacity_exceeded
      {
