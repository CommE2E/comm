...
async fn handle_insert(
  ...
  let (user_id, username) = match (user_id_attribute, username_attribute) {
    ...
    _ => {
      return Err(
        anyhow!("failed to get user_id and username from AttributeValue"),
      )
    }
  };
  ...
  let json_body = serde_json::to_string(&user_body).expect("failed to serialize user body");
...
async fn handle_modify(
  ...
  let json_body = serde_json::to_string(&update_by_query).expect("failed to serialize user body");
