...
pub enum PushType {
  /// The push type for notifications that trigger a user interaction—for example, an alert,
  /// badge, or sound.
  #[default]
  Alert,
  ...
  Voip,
  /// The push type for notifications that contain update information for a watchOS app’s
  /// complications.
  Complication,
...
pub struct NotificationHeaders {
  ...
  /// If there’s a mismatch, or if the header is missing on required systems,
  /// APNs may return an error, delay the delivery of the notification, or drop it altogether.
  pub apns_push_type: Option<PushType>,

  /// A canonical UUID that’s the unique ID for the notification.
  pub apns_id: Option<String>,

  /// The date at which the notification is no longer valid. This value is a UNIX epoch expressed
  /// in seconds (UTC). If the value is nonzero, APNs stores the notification and tries to deliver
  /// it at least once, repeating the attempt as needed until the specified date. If the value is 0,
  ...
  /// 5 - send the notification based on power considerations on the user’s device
  /// 1 - prioritize the device’s power considerations over all other factors for delivery,
