...
pub struct GenericNotifPayload {
  ...
  #[serde(skip_serializing_if = "Option::is_none")]
  pub badge_only: Option<String>,
  #[serde(skip_serializing_if = "Option::is_none")]
  pub farcaster_badge: Option<String>,
}
...
impl GenericNotifPayload {
  fn into_apns(self, device_token: &str, topic: APNsTopic) -> APNsNotif {
    ...
    if let Some(badge_only) = self.badge_only {
      payload_obj["badgeOnly"] = json!(badge_only);
    }
    if let Some(farcaster_badge) = self.farcaster_badge {
      payload_obj["farcasterBadge"] = json!(farcaster_badge);
    }
  ...
  fn into_fcm(self, device_token: &str) -> FCMMessage {
    ...
    let mut data = json!({
      ...
      "badgeOnly": self.badge_only.as_deref().unwrap_or("0"),
    });
    ...
    if let Some(farcaster_badge) = self.farcaster_badge {
      data["farcasterBadge"] = json!(farcaster_badge);
    }
  ...
  fn into_web_push(self, device_token: &str) -> WebPushNotif {
    ...
    if let Some(badge_only) = self.badge_only {
      payload["badgeOnly"] = json!(badge_only);
    }
    if let Some(farcaster_badge) = self.farcaster_badge {
      payload["farcasterBadge"] = json!(farcaster_badge);
    }
