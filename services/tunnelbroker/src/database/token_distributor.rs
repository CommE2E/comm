...
impl DatabaseClient {
  pub async fn scan_orphaned_tokens(
    ...
    if let Some(limit_val) = limit {
      unassigned_query = unassigned_query.limit(limit_val as i32);
    }

    let unassigned_future = unassigned_query.send();

    // Query expired tokens using composite index
    let mut expired_scan = self
      .client
      .scan()
      .table_name(farcaster_tokens::TABLE_NAME)
    ...
    if let Some(limit_val) = limit {
      expired_scan = expired_scan.limit(limit_val as i32);
    }

    let expired_future = expired_scan.send();
