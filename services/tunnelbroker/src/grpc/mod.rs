...
impl TunnelbrokerService for TunnelbrokerGRPC {
  async fn send_message_to_device(
    ...
    .await
    .map_err(|e| match e {
      SendMessageError::PersistenceError(ddb_error) => {
        handle_ddb_error(ddb_error)
      }
      SendMessageError::SerializationError(_) => {
        tonic::Status::invalid_argument("Invalid argument")
      }
      SendMessageError::AmqpError(lapin_error) => {
        handle_amqp_error(lapin_error)
      }
    })?;
