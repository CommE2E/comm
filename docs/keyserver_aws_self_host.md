...
Set your database name with `COMM_DATABASE_DATABASE`. You can also keep this as the default `comm`.
...
Configure your keyserver’s Comm username and password by inserting them into the empty fields in `COMM_JSONCONFIG_secrets_user_credentials`.
...
Include your Alchemy key, which is necessary for resolving ENS names in notifs generated by your keyserver. You can receive one from Alchemy’s [website](https://www.alchemy.com/).

```
COMM_JSONCONFIG_secrets_alchemy='{ "key": "" }'
```

Include your Neynar key, which is necessary for resolving Farcaster usernames in notifs generated by your keyserver. You can receive your key from Neynar’s [website](https://neynar.com/#pricing).

```
COMM_JSONCONFIG_secrets_neynar='{ "key": "" }'
```

Include your GeoIP license, which is necessary for determining client time zones based on IP addresses. You can receive one from MaxMind’s [website](https://www.maxmind.com/en/home).
...
The token values specified in your Comm APNs config are used to send push notifications to iOS and macOS devices.
...
The value for `key` should be the text from your `.p8` file. `keyID` should also be received by following the above Apple developer instructions. `teamId` can be found under Membership details. `production` remains `true`.
...
The web push config is used for web browser notifications. You can generate the necessary `publicKey` and `privateKey` with the [web-push library](https://github.com/web-push-libs/web-push).
...
On seeing the following message: `Primary service is not healthy yet. Waiting 10 seconds before checking again...`, you will need to grab your keyserver load balancer’s DNS name.
