...
The second config file contains some details that the keyserver needs in order to launch Olm sessions to provide E2E encryption. Cryptographically secure random keys will be generated by script.

```
cd server
chmod +x bash/generate_keys.sh
...
```json
{
  "privateKey": "some-random-string",
  "publicKey": "some-random-string"
}
```
